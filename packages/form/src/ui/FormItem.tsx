...
export function FormItemFc(props: FormItemProps) {
  ...
  return (
    <div data-testid={dataTestid}>
      ...
      {$children}
      {message?.error ? <div data-testid={dataTestid ? 
        `${dataTestid}-error` : undefined}>{message.error.join('-')}</div> : null}
      {message?.warn ? <div data-testid={dataTestid ? 
        `${dataTestid}-warn` : undefined}>{message.warn.join('-')}</div> : null}
    </div>
  );
}
