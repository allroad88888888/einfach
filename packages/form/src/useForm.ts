...
import type { Getter, Setter } from 'einfach-state'
import { createStore, useAtomMethods, useSetAtom } from 'einfach-state'
...
export function useForm<Values extends Obj>(props: FormProps<Values>): FormInstance {
  ...
  const methods = useAtomMethods(
    {
      setFieldValue<T>(getter: Getter, setter: Setter, name: NamePath, value: T) {
      ...
      getFieldValue<T>(getter: Getter, setter: Setter, name: NamePath) {
