...
import { createAtomFamilyStore, createStore, atom } from '../../src'
...
describe('createAtomFamilyStore', () => {
  ...
  test('应该支持自定义atom创建函数', () => {
    const { createAtomFamily } = createAtomFamilyStore()
    
    // 使用自定义的atom创建函数
    const countFamily = createAtomFamily({
      debuggerKey: 'count-family',
      createAtom: (key: string, initState?: any) => {
        return atom({
          id: key,
          count: initState?.count || 0,
          timestamp: Date.now()
        })
      }
    })
    
    const countAtom = countFamily('key1', { count: 5 })
    const state = store.getter(countAtom)
    
    expect(state.id).toBe('key1')
