...
export function createAsyncParamsAtom<T extends (...args: any[]) => Promise<unknown>>(fn: T) {
  ...
  const paramsAtom = atom<typeof uninitialized | Params>(uninitialized)

  // 创建结果atom，其值取决于参数atom的值
  const resultAtom = atom<ReturnType<T> | Promise<undefined>, Params, void>(
    (getter) => {
      // 获取当前参数
      const params = getter(paramsAtom)
      // 如果是初始状态，返回undefined
      if (params === uninitialized) {
        // return new Promise(() => { }) as ReturnType<T>
        return undefined as unknown as Promise<undefined>
      }
      // 否则，调用原始函数并传入参数，返回Promise
      return fn.call(null, ...params) as ReturnType<T>
    },
