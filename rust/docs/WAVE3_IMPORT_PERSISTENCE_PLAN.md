...
- `solid/excel/e2e/worker-workbook.spec.ts`：优先扩展这里的 import helpers，不要把 Wave 3 验收拆到新的低层 helper 里。
- `solid/excel/test/wasm-workbook-worker.test.ts`：适合补 worker 级别的 contract 断言，尤其是 `beginImport / importChunk / commitImport / cancelImport` 的状态机和错误码。
- `solid/excel/e2e/million-demo.spec.ts`：只保留 demo 级 smoke，不建议在这里承载 Wave 3 persistence 断言，避免把一套合同绑到 demo UI 上。
...
- 已在 `rust/wasm/src/lib.rs` 新增持久化 v1 核心 API：
...
- 已补充 Rust 测试覆盖：
  - v1 schema round-trip、默认 `Sheet1` 恢复、版本校验、坏 format payload 原子性、restore
    清理订阅 token。当前 `cd rust/wasm && cargo test --quiet` 为 21 passed。

## Worker / Proxy 执行记录（C1 + C3）

- `solid/excel/src/wasm-workbook-worker.ts`：
...
- `solid/excel/src/wasm-workbook-proxy.ts`：
...
- Playwright CLI：
  - `cd solid/excel && npm run e2e -- e2e/worker-workbook.spec.ts --grep "persistence|import"`
  - 结果：9 passed。
- MCP Playwright：
...
- `solid/excel/wasm-pkg/` 被 `.gitignore` 忽略；本地浏览器/e2e 需要在 Rust 改动后运行
