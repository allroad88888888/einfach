...
- **位置**：`rust/core/src/store.rs:21-37`
...
### A.5（中）`set` 没有写循环检测
- **位置**：`rust/core/src/store.rs:179-219`
...
- **位置**：`rust/core/src/store.rs:105-148`
...
- **位置**：`rust/core/src/store.rs:223-232`
...
## B. Rust excel-core (`rust/excel-core`)

### B.1（必修，已验证）`set_formula` 的 `cell_map` 是快照 — 后建公式的 cell 不会传播到先建的引用方
- **位置**：`rust/excel-core/src/sheet.rs:65-89`
...
- **位置**：`rust/excel-core/src/sheet.rs:58-89`
...
- **位置**：`rust/excel-core/src/sheet.rs:60`
  ```rust
...
- **位置**：`rust/excel-core/src/eval.rs:141-190`
- **现状**：
...
- **位置**：`rust/excel-core/src/eval.rs:225, 241`
...
- **位置**：`rust/excel-core/src/sheet.rs:82-84`
  ```rust
...
## D. Solid Excel (`solid/excel`)

### D.1（必修，影响首屏）"可切换后端"目标 vs 实际：JS 后端跟 Rust 行为不等价 — **首屏 demo 已显示错误结果**
- **位置**：`solid/excel/src/js-sheet.ts:25-65`，入口 `solid/excel/src/App.tsx:26` 默认 tab 是 `formulas`
...
- **位置**：`solid/excel/src/js-sheet.ts:54`
  ```ts
...
- **位置**：`solid/excel/src/sheet-store.ts:28-39`
...
- **位置**：`solid/excel/src/sheet-store.ts:9-17`
...
- **位置**：`solid/excel/src/sheet-store.ts:9`
- **影响**：长生命周期 + 滚动浏览大量 cell 时单调增长
- **改法**：用 LRU 限制大小；或者组件卸载时主动 remove

### D.6（中）`store.raw` 暴露内部 ISheet 引用
- **位置**：`solid/excel/src/sheet-store.ts:83`
- **风险**：用户绕过 store 直接 `raw.set_number(...)` 不会触发 refresh，signal 跟 sheet 状态分裂
- **改法**：删除 `raw` 字段；如有诊断需求改成 `__unsafe_raw`

### D.7（中）没有暴露 batch
- **位置**：`solid/excel/src/sheet-store.ts`
- **现状**：N 次连续 set 触发 N 次 refreshAll
- **改法**：加 `batch(fn)` 包一层 sheet.batch + 单次 refresh

### D.8（轻）Cell focus 用 `setTimeout(() => el.focus(), 0)`
- **位置**：`solid/excel/src/Cell.tsx:52`
- **改法**：用 SolidJS 的 `onMount` 或 `requestAnimationFrame`

### D.9（轻）`Cell` 内多次调用 `cellValue()` 触发多次 `getCell`
- **位置**：`solid/excel/src/Cell.tsx:13, 39, 44`
- **改法**：`const value = createMemo(() => props.store.getCell(props.addr))`

### D.12（轻，已用 pragma 临时绕过）`solid/excel` 的 src .tsx 缺 `@jsxImportSource solid-js` pragma → `npm run build` 卡 tsc
- **位置**：`solid/excel/src/*.tsx` + `solid/excel/src/demos/*.tsx`（10 个文件）
- **根因**：根 `tsconfig.json` 用 `"jsx": "react-jsx"`，且没把 `solid/excel/tsconfig.json` 加到 `references`。`solid/excel/test/*.tsx` 通过 import 把 src 的 .tsx 拉进根 tsconfig 编译范围 → 用 React JSX 类型检查 SolidJS 代码 → 报错
- **临时修法（已实施）**：每个 src .tsx 顶部加 `/** @jsxImportSource solid-js */` pragma（跟 test 文件一致）
- **正式修法（建议放 1A step 10 vite + wasm 一起做）**：
  1. `solid/excel/tsconfig.json` 改成 extends `tsconfig.base.json` 风格（参考 `solid/solid/tsconfig.json`），加 `composite: true`
...
- **位置**：`solid/excel/src/Cell.tsx:16` `setEditValue(cellValue().display)` + `Cell.tsx:21` `setCellInput(editValue())`
- **现象**：B1 = `=A1*2`，display 是计算结果 `"20"`。用户双击 B1 进入编辑，input 里显示 `"20"`，回车提交 → `setCellInput("20")` → `set_number(20)` → 公式被静态值替换
- **根因**：`solid/excel/src/types.ts:2` 的 `ISheet` 接口没有 `get_formula(addr)` 方法；`rust/wasm/src/lib.rs` 也没暴露
...
- **位置**：`solid/excel/src/demos/DemoFormulas.tsx:11-83`
...
4. **TS 类型生成 vs 手写 ISheet**：当前 `solid/excel/src/types.ts` 手写 `ISheet`。WASM 编出来后 wasm-bindgen 自动生成 `.d.ts`，需要决定是否抛弃手写、改用生成的类型，还是把 `ISheet` 当成稳定契约

### E.4（中）第七期 Web Worker 跟当前架构的耦合点没说清
- `rust/core/src/store.rs:40-44` 用 `thread_local!` 存 TRACKING / COMPUTING
