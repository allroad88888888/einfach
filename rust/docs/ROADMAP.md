...
- 第一期已完成：键盘交互、公式栏、输入态保留、WASM 订阅链路已接通
- 第二期已完成：比较运算、逻辑/数学/文本函数、`^` / `&` 运算符、`COUNTIF` / `SUMIF`
- 第三期已完成：范围选择、复制/剪切/粘贴、Delete 清空、撤销重做、绝对引用偏移
- 第四期已完成：行列插删、宽高调整、冻结首行/首列、多 Sheet、跨 Sheet 引用、Workbook/WASM 对齐
- 第六期主线已完成：单元格格式、CSV/JSON 导入导出、localStorage 自动保存
- 下一期待定：条件格式、查找函数与数据处理
...
- 新运算符：`^`（幂）、`&`（文本拼接）
- 逻辑函数：`AND`, `OR`, `NOT`
- 数学函数：`ABS`, `ROUND`, `CEILING`, `FLOOR`, `SQRT`, `POWER`, `MOD`
- 文本函数：`CONCATENATE`, `LEN`, `LEFT`, `RIGHT`, `MID`, `UPPER`, `LOWER`, `TRIM`, `TEXT`
- 条件聚合：`COUNTIF`, `SUMIF`
- Rust / JS fallback / WASM 相关单测已通过
...
### 本期已交付
- 范围选择：点击、`Shift+点击`、`Shift+方向键`
- 复制粘贴：内部结构化剪贴板 + 外部 TSV 互通
- 剪切：`Ctrl/Cmd+X` 后源区清空，可撤销恢复
- 撤销重做：`Ctrl/Cmd+Z`、`Ctrl/Cmd+Y`、`Ctrl/Cmd+Shift+Z`
- Delete 清空：空输入统一视为清空单元格
- 公式偏移：支持相对引用与 `$A$1` / `$A1` / `A$1` 绝对引用
- Rust / WASM / JS fallback / Solid 交互测试已通过
