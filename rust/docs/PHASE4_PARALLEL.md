...
`solid/excel/src/Table.tsx` today has a hand-rolled row virtualizer
...
| **O** | 1M-cell worker-backed demo | New `solid/excel/src/demos/DemoMillion.tsx` (1000×1000 or 10000×100, both shapes worth exercising). Worker-backed sheet (existing `createWorkerSheet`). A scattered seed (few thousand cells across the 1M coord space) so the lazy-eval + sparse range stories show. Add to the `App.tsx` nav under `perf` group. | 1–2 d | parallel with M (different files) |
...
- `solid/excel/src/Table.tsx` (heavy rewrite)
- `solid/excel/package.json` (one dep add)
...
New file `solid/excel/src/demos/DemoMillion.tsx`:
...
- `solid/excel/src/demos/DemoMillion.tsx` (new)
- `solid/excel/src/App.tsx` (one new tab entry)
- `solid/excel/src/i18n/locales/en.ts` (3 new keys)
- `solid/excel/src/i18n/locales/zh.ts` (3 new keys)
...
- `solid/excel/e2e/virtualize.spec.ts` (extend existing)
- `solid/excel/e2e/million-demo.spec.ts` (new — Million-demo-specific)
