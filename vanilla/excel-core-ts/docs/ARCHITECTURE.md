...
```
┌──────────────────────────────────────────────────────────────┐
│ vnext UI (solid/excel/src-vnext/)                            │
...
└─────────────────┬────────────────────────────────────────────┘
                  ...
                  ▼
┌──────────────────────────────────────────────────────────────┐
│ Worker (solid/excel/src-vnext/adapter/worker-runtime.ts)     │
...
└─────────────────┬────────────────────────────────────────────┘
                  ...
                  ▼
┌──────────────────────────────────────────────────────────────┐
│ @einfach/core (vanilla/core)                                 │
│  - createStore, atom, sub                                    │
│  - WeakMap-based dep tracking + lazy invalidation            │
└──────────────────────────────────────────────────────────────┘
```

Key property: **`@einfach/excel-core` does not import `solid-js`, the DOM, or anything UI-specific**. Same discipline as `vanilla/spreadsheet-ui-core`. It depends only on `@einfach/core`.
...
The atom is **the entire sheet's cell map**. Writes go through a setter that takes a structured mutation (`SetCell`, `ClearCell`, `BulkApply`, etc.), applies it immutably, and produces a new `Map` reference. `vanilla/core`'s `Object.is` cache check then invalidates every formula derive in one stroke.

Why immutable? `dep-equality` in `vanilla/core` is reference equality. If we mutated the existing Map, derives wouldn't see a change. The new-Map allocation is cheap (~50 ns/op even for 10k-cell sheets).
...
workbook.setCell(sheetId, key, input)
     ...
     ▼
vanilla/core: marks every back-dep of sheetAtom dirty
     │
     ▼
...
**No explicit recalc loop in the core.** vanilla/core's `flushPending` does the work. Mutation = single setter call. Evaluation = whatever happens when subscribers read.
...
The set is scoped to a **single top-level `evaluate()` call chain**, not global. Recursive `refLookup` within the same evaluation share the set; once the chain unwinds, the set is empty. Concurrent evaluations from different `get` paths get fresh sets — vanilla/core serializes derive execution so concurrency isn't a worry.
...
This registers a dep on the **other sheet's atom** — vanilla/core handles cross-atom propagation natively. When Sheet2 changes, formulas on Sheet1 referencing Sheet2 invalidate. No special cross-sheet bookkeeping.
...
Async custom formulas (returning a Promise) integrate via `vanilla/core`'s built-in promise support — the derive returns a `StatesWithPromise<Value>` and subscribers see the placeholder, then the resolved value.
...
## 12. What stays in `vanilla/spreadsheet-ui-core`
...
## 13. What stays in `solid/excel/src-vnext/adapter/`
