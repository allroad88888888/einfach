...
`solid/excel/src-vnext/grid/SpreadsheetGrid.tsx`); the chrome is a flat
...
- `solid/excel/src-vnext/name-box/SpreadsheetNameBox.tsx`, mounted in
  the formula-bar row, left of
  `solid/excel/src-vnext/formula-bar/SpreadsheetFormulaBar.tsx`. The
...
`solid/excel/src-vnext/status-bar/SpreadsheetStatusBar.tsx` currently
...
- `solid/excel/src-vnext/status-bar/SpreadsheetStatusBar.tsx` (modified)
- `solid/excel/src-vnext/status-bar/AggregatePicker.tsx` (new)
- `solid/excel/src-vnext/status-bar/ZoomSlider.tsx` (new)
...
- `solid/excel/src-vnext/toolbar/SpreadsheetToolbar.tsx` adds a
  `format-painter` command to `toolbarCommands`, with `onDoubleClick`
  entering sticky mode.
- `solid/excel/src-vnext/grid/SpreadsheetGrid.tsx` reads
...
`solid/excel/src-vnext/grid/overlay/OverlayRenderer.ts` is constructed
...
- `spreadsheetProjectionSnapshotAtom` (host atom in
  `solid/excel/src-vnext/provider/atoms.ts`, for `conditionalFormat` /
  `mergedSpan` data)
- `zoomLevelAtom`
...
New file: `solid/excel/test/vnext-grid-overlay.test.tsx`.
...
- **Modified files ~5.** `SpreadsheetGrid.tsx` (mount overlay, format-painter
  ...
  `formula-bar-addr`); `vanilla/spreadsheet-ui-core/src/index.ts`
  (re-exports).
...
- New: `test/menu-bar.test.ts`, `test/name-box.test.ts`,
  ...
  `solid/excel/test/vnext-grid-overlay.test.tsx`.
