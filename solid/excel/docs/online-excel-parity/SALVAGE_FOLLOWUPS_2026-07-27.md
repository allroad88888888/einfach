...
`solid/excel/src-vnext/adapter/static-formula-eval.ts` 的 tokenizer **没有**
表达式内裸错误字面量的语法规则，整条公式 tokenize 失败，单元格读成 `#ERROR!`。

**证据**：`solid/excel/test/vnext-auto-fill-static-wasm-parity.test.ts` 场景 7，
文件头与用例上均有 FIXME，当前按"以 WASM 为准"分别钉住两侧现值。

**解决方案**：
1. 在 static-formula-eval 的 tokenizer 里加一类 `ErrorLiteral` token，
   识别集合与 `rust/wasm/src/lib.rs::error_token_to_value_error` 的表对齐：
...
**验收**：parity 套件场景 7 双侧一致且无 FIXME；`npx jest solid/excel` 全绿。
...
但 `rust/wasm/src/lib.rs` 的 wire 反序列化只认 `"decimal"`；传 `"number"` 时
...
4. 记得同 commit 内跑 WASM API 快照门禁（若签名有变）：
   `cd rust/excel-core && cargo test --test architecture_invariants wasm_snapshot_generate -- --ignored`
...
### 3. `rules/.eslintrc` 漏配 `solid/*`，整个 solid/excel 从未被行级 lint

**现状**：`parserOptions.project` 列表缺 `./solid/*/tsconfig.json`，导致所有
`solid/excel/**` 文件在 `npm run eslint` 与定向 `npx eslint --config rules/.eslintrc`
下**解析直接失败**（在完全未改动的 `solid/excel/src-vnext/public.ts` 上可复现），
...
2. 先做**只读**定向扫描评估规模（切勿直接跑 `npm run eslint`——它是全仓 `--fix`，
   会顺手改动 apps/ 等并行工作区）：
   `npx eslint --config rules/.eslintrc --ignore-path rules/.eslintignore 'solid/excel/**/*.{ts,tsx}'`
...
`solid/excel/src-vnext/adapter/worker-runtime.ts:368` 的
...
- worker-runtime.ts:368：把 `CustomFormulaCallable` 的参数类型对齐
  ...
  `rust/excel-core/src/CUSTOM_FORMULAS.md` § Marshaling），或让 pump 的
...
**解决方案**：
1. 新建 `vanilla/spreadsheet-ui-core/src/internal/ack-hardening.ts`
...
**验收**：`grep -rn 'function snapshotAcknowledgement' vanilla/spreadsheet-ui-core/src`
...
3. **引擎**：`sheet.rs` 的 `call_custom` 加 REMOTE 前置分支（走 async-custom
   ...
   要接进 `rust/wasm` 的 error token 表**双向**（`error_token_to_value_error`
