...
7. Full Solid `tsc` 最终命令 `npx tsc --noEmit --pretty false -p solid/excel/tsconfig.json` exit 2 / 恰好 5 diagnostics，均位于未修改且禁止扩围的 `worker-runtime*` baseline；C1 新测试 typing 已修，仍不得写成 `tsc PASS`。
...
10. `vanilla/core`、`vanilla/excel-core-ts`、`rust/**` 是本次迁移保护边界，当前 Agent 不得为了 UI parity 重写 engine。
...
| `vanilla/spreadsheet-ui-core` | `@einfach/core` Source/Derived/Command atoms、独立 store、有界 projection、严格 lifecycle | 引入其他状态库、Solid/DOM 依赖、无界产品事实缓存                              |
...
| released | `/root/docs_evidence_refresh`                      | `solid/excel/docs/online-excel-parity/**`                                   | `Docs Evidence MAIN_REVIEW_ACCEPTED / released`；主审已接受                                                                                                                                                                   | 不改源码、不预宣称代码路线成功                                                                                                                                  |
...
| `vanilla/spreadsheet-ui-core` | 用 `@einfach/core` 定义 Source / Derived / Command atoms、状态机、有界缓存和 backend ports | 依赖 Solid；创建第二套 parser；使用无界 module `Map` 保存产品事实                            |
...
| ---------------------------------------------------------- | ---------------------- | ------------------------------------------------- | -------------------------------------------------------------- |
| `solid/excel/src-vnext/grid/SpreadsheetGrid.tsx`           | `/root`                | 提交单一交互入口的定向 patch 与对应 grid/E2E 证据 | 检查 projection 有界、UI 无第二状态源、事件只派发 core command |
| `solid/excel/src-vnext/adapter/worker-runtime.ts`          | `/root`                | 提交单一 RPC/能力接线 patch 与双引擎证据          | 检查 TS/Rust 语义、optional capability 和错误合同              |
| `solid/excel/src-vnext/adapter/worker-runtime-ts.ts`       | `/root`                | 提交与 wasm runtime 同合同的定向 patch            | 与 `worker-runtime.ts` 串行接入并跑 conformance                |
| `solid/excel/src-vnext/adapter/worker-protocol.ts`         | `/root`                | 提交协议字段提案、兼容性说明和调用点清单          | 公共协议冻结后才允许实现；不得专题私自扩展                     |
| `solid/excel/src-vnext/adapter/worker-workbook-backend.ts` | `/root`                | 提交 backend port 接线 patch 与窗口化读取证据     | 保留 capability 和有界 projection，不复制权威事实              |
| `solid/excel/src-vnext/provider/SpreadsheetUiProvider.tsx` | `/root`                | 提交 provider 生命周期 patch 与 teardown 测试     | Provider 只桥接 store/backend；产品状态仍归 `@einfach/core`    |
...
```bash
git diff --check -- solid/excel/docs/online-excel-parity
npx prettier --check solid/excel/docs/online-excel-parity/*.md
```
...
影响 `solid/excel/src-vnext` adapter、provider 或 UI：
