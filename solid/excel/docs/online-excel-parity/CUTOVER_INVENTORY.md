...
| released | #29 筛选隐藏语义重做（任务 #27，S1–S8）           | 压缩→隐藏原子翻转 + 引擎双隐藏集 + 可见性 UI-core canonical + 导出/删除收口 | 九提交（`7c13fd4`…`50a14e1`）+ 本文档收口；ui-core 63 suites / 1583 tests、solid 97 suites / 1455 tests（1 suite / 6 tests skipped）；`Data → Reapply` 入口未落地，是唯一在案缺口；#29 仍 `Partial`                                                                                        |
...
用户可见变化四条，两条需进发布说明：**粘贴/填充会写入被筛掉的行**（Excel 就是连续块覆写，是向 Excel 收敛的 parity **修复**，但对习惯旧行为的用户可感知）；**筛选不再实时**——改单元格值不会让行当场消失/出现（快照语义，与 Excel 一致，压缩语义反而比 Excel 更"活"）。另两条：行号跳号（产品目的）、筛选区合并单元格重新可见。**唯一在案缺口：`Data → Reapply` 入口未落地**——设计把 `reapplyFilterAtom` + Data 菜单项 + `Ctrl+Alt+L` 派进 S7，三者均未实现（2026-07-21 核实：无该原子导出、无菜单项、无键位），今天只能靠重开列下拉重发规则来刷新快照。
