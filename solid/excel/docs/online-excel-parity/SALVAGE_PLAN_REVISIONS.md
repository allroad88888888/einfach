...
**实测**：`vanilla/spreadsheet-ui-core/node_modules/@einfach/`（以及所有其它包的同名目录）
...
npx jest solid/solid/test/provider-remount.test.tsx \
         solid/excel/test/provider-remount-1912.test.tsx --no-coverage
```
...
`rules/.eslintrc` 加 `./solid/*/tsconfig.json` 后，solid/excel 首次能被解析。
...
`vanilla/spreadsheet-ui-core/src/auto-fill/command.ts:1146-1166`，已随 `fe88a3f` 提交，
...
W2  F helper 已建；6 模块回迁降级为不强制
     ↓
W3  C-later（solid/excel 170 条存量，先清 no-console）
     G 不做（理由改正）
     ↓
W4  REMOTE 待排期
```
