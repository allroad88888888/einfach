...
|  03 | 行高、列宽、隐藏/取消隐藏                | `Partial`  | 混合：隐藏/取消隐藏 = UI-core 视图事实（已翻转）；行高列宽 = 引擎数据事实（as-built 修正）  | 隐藏从"等 Worker hidden capability"改为 **UI-core canonical 翻转**（顺序第 2 步，已落地）；行高列宽经 hidden 切片耦合审计确认引擎真实实现且与 hidden 0 尺寸数学正交，**维持 backend canonical，不翻转**（2026-07-19 as-built 修正） |
...
| 混合               |     12 | #01 03 15 26 29 31 34 35 36 37 38 41                                            |
...
1. **SUBTOTAL 101-111 与 hidden —— 裁决：宿主喂可见性给引擎。** #32 实现时采用"求值上下文推送"：宿主把 per-sheet 隐藏行集合作为只读求值输入喂给引擎（可选推送端口或求值参数），引擎不建 hidden 模型、不感知隐藏来源（手动/filter 同一集合）。不采用宿主侧求值（避免在宿主复刻聚合语义）。在 #32 落地前，现状"101-111 折算为 1-11"记为已知 conformance gap，不阻塞其他项。
