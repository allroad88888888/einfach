...
`excel/rust/excel-core/src/workbook.rs` 已经有 `CrossSheetRef::Range`、每 source
sheet 一个 `RangeDependentIndex`、dirty fanout、cycle BFS 分支；测试
`excel/rust/excel-core/tests/cross_sheet.rs::cross_sheet_range_dirty` 也已经写好。
...
|---|---|---|---|---|
| **A** | Parser AST | `excel/rust/excel-core/src/formula.rs` | 0.5–1 d | 可先行 |
| **B** | Workbook integration | `excel/rust/excel-core/src/workbook.rs`, `excel/rust/excel-core/src/eval.rs` | 0.5–1 d | 依赖 A 的 AST 形状 |
| **C** | Tests / un-ignore | `excel/rust/excel-core/tests/cross_sheet.rs`, formula parser tests | 0.5 d | 可先写 red tests，最终等 A/B |
...
```sh
cd excel/rust/excel-core && cargo test --test cross_sheet cross_sheet_range_dirty -- --exact
cd excel/rust/excel-core && cargo test --lib workbook
```
...
cd excel/rust/excel-core && cargo test --lib formula
