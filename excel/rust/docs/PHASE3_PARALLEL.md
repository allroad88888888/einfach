...
| **K** | WASM workbook API | `excel/rust/wasm/src/lib.rs` `WasmWorkbook`: expose `set_cell` / `set_formula` / `clear_cell` / `bulk_load` / `subscribe_cell` (the per-cell subscriber that fires on cross-sheet writes). | 2 d | parallel with I (different file) |
...
**Concrete deliverables on `WasmWorkbook` (excel/rust/wasm/src/lib.rs):**
...
**Files K owns:** `excel/rust/wasm/src/lib.rs` (additive).

**DO NOT touch:** any `excel-core/src/*`, frontend, docs.

## Track L — Tests + Bench

**New file `excel/rust/excel-core/tests/cross_sheet.rs`** (separate from
