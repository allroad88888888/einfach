...
> then run the quick-verify block (§8). `excel/rust/excel-core/tests/architecture_invariants.rs`
...
`excel/rust/excel-core` bypassed the store's derived mechanism entirely and grew
...
4. **No perf red line for this arc.** Correct mechanism first; measure after
   ...
   deviations (§6).
5. TS engine (`excel/excel-core-ts`) is NOT the architectural reference —
...
| P2 | `AtomFamily` core primitive | all green, zero changes outside excel/rust/core |
...
# browser gate; see excel/rust/wasm/README.md if wasm-pack has a stale ChromeDriver cache
wasm-pack test --headless --chrome ../wasm
cd ../.. && npx jest excel/solid-excel --no-coverage
# tripwire
cd excel/rust/excel-core && cargo test --test architecture_invariants
```

Baseline counts (P0, recorded 2026-07-07 at commit 208688d):
- excel/rust/core: 65 passed / 0 ignored
- excel/rust/excel-core `--lib`: 1404 passed / 3 ignored
- excel/rust/excel-core integration (incl. new golden_replay + architecture_invariants): ~1825 passed / 12 ignored total across 65 test binaries, 0 failed
- excel/rust/wasm native: 31 passed / 1 ignored (browser: 13 wasm-pack tests, run separately)
...
P7 final validation (2026-07-10): the full `excel/rust/core`, `excel/rust/excel-core`
...
workaround in `excel/rust/wasm/README.md`.
...
**Verification at P4 exit (unchanged from §4 owner-approved table):** excel/rust/core
...
`cell_dependents`/`mark_dependents_dirty`; jest excel/solid-excel; e2e Δ=0; playwright
MCP UI walkthrough.
