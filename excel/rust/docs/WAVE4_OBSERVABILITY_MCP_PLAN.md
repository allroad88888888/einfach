...
- `excel/solid-excel` 已有 viewport subscription 测试：
  - `virtualize.spec.ts`
  - `million-demo.spec.ts`
  - `observe-cell.test.ts`
- Wave 3 已用 MCP Playwright 验证 import cancel、import limit、persistence lazy restore。
- `excel/solid-excel/docs/E2E_TEST_PLAN.md` 已是 2026-05-13，但数值已变为 21 spec / 150 tests；
...
| 角色 | 推荐模型 | 文件所有权 | 任务 |
|---|---|---|---|
| D1 Rust/WASM counters | `gpt-5.3-codex-spark` | `excel/rust/excel-core/src/*`, `excel/rust/wasm/src/lib.rs`, Rust tests | 补 workbook live subscription / formula count / session-relevant counters 的最小 WASM 暴露，保持 lazy |
| D2 Browser perf/MCP | Claude Sonnet（只读审查）+ 总架构师 | `excel/solid-excel/e2e/*`, MCP 记录 | 设计可重复 MCP 脚本：DOM 数、eval count、subscription bound、console |
| D3 Docs gate | `gpt-5.3-codex-spark` 或便宜 Codex | `excel/solid-excel/docs/E2E_TEST_PLAN.md`, `excel/rust/docs/HANDOFF.md`, 本计划 | 重新统计 spec/test/skip，更新真实状态和 MCP 流程 |
| D4 Test guard | `gpt-5.3-codex-spark` | `excel/solid-excel/e2e/*`, `excel/solid-excel/test/*` | no-new-skip/no-only 扫描，补缺少的稳定 e2e 或 Jest 断言 |
...
```sh
cd /Volumes/work/self/einfach && rg --line-number "test\\.only|describe\\.only|it\\.only|test\\.skip|describe\\.skip|it\\.skip" excel/solid-excel/e2e excel/solid-excel/test
cd /Volumes/work/self/einfach && npx tsc -p excel/solid-excel/tsconfig.json --noEmit
cd /Volumes/work/self/einfach && npx jest excel/solid-excel/test/wasm-workbook-worker.test.ts excel/solid-excel/test/worker-workbook-store.test.ts excel/solid-excel/test/observe-cell.test.ts --runInBand
...
- `cd excel/solid-excel && npx playwright test --list`：
  - 21 spec files；
  - 150 active tests；
  - 0 active skip/only。
...
- 新增 `excel/solid-excel/e2e/observability.spec.ts`：
...
- `cd excel/solid-excel && npm run e2e -- e2e/observability.spec.ts`：
  - 2 passed。
...
- `excel/solid-excel/docs/E2E_TEST_PLAN.md` 的计数与真实 `playwright test --list` 一致。
- `excel/rust/docs/HANDOFF.md` 和 `ONLINE_SPREADSHEET_EXECUTION_WAVES.md` 更新到 Wave 4 当前状态。
- 最终汇报必须列出：
