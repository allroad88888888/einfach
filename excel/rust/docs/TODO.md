...
- 仍在 deferred：5 个 demo 页 (`excel/solid-excel/src/demos/*`) 没逐页加 `formulaBar` prop —— 用户得改 prop 调用方才看到
...
- **重现**：`excel/solid-excel/e2e/smoke.spec.ts` "Ctrl/Cmd+Z undoes" 测试目前要按两次 Meta+Z 才能回到空，已在测试里注释标注
...
- **测试**：`excel/solid-excel/test/sheet-store.test.ts` 加 12 个用例（selection range 5 + TSV helper 5 + copy/paste roundtrip 2）
- **未做 / 已知限制**：
...
- **已落地**（JS-side mock）：
  - `excel/solid-excel/src/workbook-store.ts` — `createWorkbookStore()` 镜像了 Rust `Workbook` 的 add/rename/remove/sheet/active 语义；每张 sheet 是 `createSheetStore(createJSSheet())`
  - `excel/solid-excel/src/SheetTabs.tsx` — 底部 tab 栏，点击切换 / `+` 新增 / 右键 prompt 改名+删除
  - `excel/solid-excel/src/demos/MultiSheet.tsx` — 用 `Show keyed` 在 active 切换时重挂 Table，3 张预填 sheet
  - 单元测试：`excel/solid-excel/test/workbook-store.test.ts`（16 用例）
...
- **仍缺**：
  - `excel/rust/wasm/src/lib.rs` 暴露 `WasmWorkbook`（add_sheet / sheet_at / sheet_count / rename_sheet / remove_sheet / get_cell 跨 sheet）
...
- **plumbing**：`excel/solid-excel/vite.config.ts` 加了 `vite-plugin-wasm` + `vite-plugin-top-level-await`；`excel/solid-excel/src/wasm-sheet.ts` 包 wasm-pack 输出（`./wasm-pkg/`，`.gitignore` 已忽略）；`build:wasm` script 跑 `wasm-pack build --target web --out-dir ./wasm-pkg ../../rust/wasm`
...
### 2.1 ✅ tsconfig 治理 — 已做
- `excel/solid-excel/tsconfig.json` extends base + 加进根 references，src .tsx 删除了 `@jsxImportSource` pragma
...
- **落地**：`excel/rust/wasm/tests/web.rs` + `wasm-bindgen-futures` dev-dep
...
- **跑法**：`wasm-pack test --headless --chrome excel/rust/wasm`（或 `--node` 作 fallback，详见 `excel/rust/wasm/README.md`）
- **后续**：unsubscribe 跨边界 / 订阅风暴重入保护可在同一 harness 继续加测，已留 hook

### 2.4 ⚠️ playwright e2e 套件
- **现状**：`excel/solid-excel/e2e/smoke.spec.ts` 落地，覆盖 cell 编辑提交 / 公式提交 / 依赖传播 / undo / redo / FormulaBar 公式源同步 / 键盘导航（Arrow+Tab+Shift+Tab），共 7 个测试。`@playwright/test` 已加入 devDependencies；`excel/solid-excel` 下 `npm run e2e` 启动 vite dev server + 跑测试
...
- **未做**：`batch_set` 写入 Null 时批量释放 primitive（当前仍会留下 atom；下次显式 `clear_cell` 才释放）
- **影响**：长期运行 set→clear 循环不再增长 atom 数；仍被 core derived 依赖的 primitive 会保留，避免 `destroy_atom` panic
