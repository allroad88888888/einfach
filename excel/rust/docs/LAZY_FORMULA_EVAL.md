...
/ `mem::transmute` 在 `excel/rust/excel-core/src/` 现在 0 命中。跨 sheet 通道
...
3. 跑一遍 `excel/solid-excel` demo 的 5 个页面 + DemoFormulas，确认无可见回归（Solid per-cell tick signal 已经按 dirty 通知设计，但要实测）
...
- ✅ grep `with_cross_resolver` / `CrossSheetResolver` / `thread_local!` / `mem::transmute` / `CROSS_RESOLVER` / `CURRENT_SHEET` / `CROSS_SHEET_VISITED` 在 `excel/rust/excel-core/src/` 均 0 命中
...
- ✅ sheet 公式路径不再调用 `Store::create_derived` / `Store::propagate_force`
  （`grep create_derived excel/rust/excel-core/src/*.rs` == 0）
...
验收 grep 结果（在 `excel/rust/excel-core/src/`）：
- `create_derived` 0 命中
...
| Legacy TLS resolver 已删除 | ✅ grep `with_cross_resolver` / `thread_local!` / `mem::transmute` / `CrossSheetResolver` / `CROSS_RESOLVER` / `CURRENT_SHEET` / `CROSS_SHEET_VISITED` 在 `excel/rust/excel-core/src/` 均 0 命中 |
