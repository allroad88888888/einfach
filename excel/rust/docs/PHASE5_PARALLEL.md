...
- `excel/solid-excel/src/wasm-sheet-worker.ts` 仍只持有一个 `WasmSheet`，不是
  `WasmWorkbook`。
- `excel/solid-excel/src/wasm-sheet-proxy.ts` 仍是 fire-and-forget 协议；`set_formula`
...
|---|---|---|---|---|
| **A** | Worker Workbook RPC | `excel/rust/wasm/src/lib.rs`, new `excel/solid-excel/src/wasm-workbook-worker.ts`, new `excel/solid-excel/src/wasm-workbook-proxy.ts`, protocol types | 2–3 d | 先行，其他 Track 依赖它的协议 |
| **B** | Typed Import / Sparse Snapshot | `excel/rust/wasm/src/lib.rs`, `excel/rust/excel-core/src/csv.rs`, worker import commands, optional persistence JSON | 2–3 d | 依赖 A 的 request/reply，可与 C 读代码并行 |
| **C** | Frontend State / Range-Native UI | `excel/solid-excel/src/sheet-store.ts`, `Table.tsx`, workbook demos, worker workbook store | 3–4 d | A 的最小 client 出来后开始 |
| **D** | E2E / Bench / Docs Gatekeeper | `excel/solid-excel/e2e`, `excel/solid-excel/test`, `excel/rust/excel-core/tests`, docs | 2–3 d | 全程并行；先写 red tests，最后解除/更新 |

## 文件冲突矩阵

|  | A | B | C | D |
|---|---|---|---|---|
| **A** | — | 都会 touch `excel/rust/wasm/src/lib.rs`，A 先定 protocol/canonical API，B 追加 import methods | C 只消费 A 新 client，不改 A worker internals | D 可先写 fake-worker tests，最终按 A 协议调整 |
...
cd excel/solid-excel && npm run build
cd /Volumes/work/self/einfach && npx jest excel/solid-excel/test/wasm-workbook-proxy.test.ts
cd excel/solid-excel && npx playwright test e2e/worker-workbook.spec.ts
```
...
cd excel/rust/excel-core && cargo test --lib csv
cd excel/rust/excel-core && cargo test --test cross_sheet
cd excel/rust/wasm && cargo build
cd excel/solid-excel && npx playwright test e2e/import-worker.spec.ts
```
...
cd /Volumes/work/self/einfach && npx jest excel/solid-excel/test/workbook-store.test.ts
cd excel/solid-excel && npx playwright test e2e/range-ops.spec.ts e2e/million-demo.spec.ts
cd excel/solid-excel && npx playwright test e2e/workbook-chain.spec.ts
```
...
cd excel/solid-excel && npx playwright test e2e/worker.spec.ts e2e/workbook-chain.spec.ts e2e/million-demo.spec.ts
```
