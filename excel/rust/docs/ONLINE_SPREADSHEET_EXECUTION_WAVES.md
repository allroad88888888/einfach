...
- 发布门禁已提交到 `e5a25d0`：Rust/WASM/Jest/Solid/Playwright/MCP 全量结果记录到
  `excel/rust/docs/RELEASE_GATE_PLAN.md`。
...
- `excel/solid-excel` 已有 worker-owned workbook RPC、request id、统一 ok/error envelope、
...
| 角色 | 推荐模型 | 文件所有权 | 任务 |
|---|---|---|---|
| A1 Worker 协议复核 | Claude Sonnet | `excel/solid-excel/src/wasm-workbook-proxy.ts`, `excel/solid-excel/src/wasm-workbook-worker.ts` | 只读/小补丁：确认 ok/error/code、request version、hydrate 乱序没有漏测 |
| A2 Store 合同收口 | Codex Spark | `excel/solid-excel/src/wasm-workbook-store.ts`, 必要时 `excel/solid-excel/src/types.ts` | 清点仍调用同步 `set_formula` 的产品入口；能改则改到 async command facade |
| A3 测试 | Codex Spark | `excel/solid-excel/test/*`, `excel/solid-excel/e2e/worker-workbook.spec.ts` | 钉非法公式、循环、乱序 hydrate、cache probe 的回归测试 |
| A4 文档 | Codex Spark 或便宜 Codex | `excel/rust/docs/HANDOFF.md`, `excel/solid-excel/docs/E2E_TEST_PLAN.md`, 本文档 | 同步 worker authority 当前状态和 agent 调度记录 |
...
cd excel/solid-excel && npx playwright test e2e/worker-workbook.spec.ts e2e/workbook-chain.spec.ts
```
...
| 角色 | 推荐模型 | 文件所有权 | 任务 |
|---|---|---|---|
| B1 Rust/WASM range format undo | Claude Sonnet | `excel/rust/excel-core/src/sheet.rs`, `excel/rust/wasm/src/lib.rs` | 设计 range format layer snapshot/restore 或 compaction transaction，不触发公式 eval |
| B2 Worker streaming export | Codex Spark | `excel/solid-excel/src/wasm-workbook-proxy.ts`, `excel/solid-excel/src/wasm-workbook-worker.ts` | 从 `exportRangeTsv` 一次性字符串推进到 chunked export |
| B3 SheetStore range UX | Codex Spark | `excel/solid-excel/src/sheet-store.ts`, `excel/solid-excel/src/Table.tsx` | large format undo、copy/export streaming 接入；维持小范围路径 |
| B4 E2E/Perf | Codex Spark | `excel/solid-excel/e2e/range-ops.spec.ts`, `excel/solid-excel/e2e/million-demo.spec.ts` | 钉住不展开百万地址、不清空旧 undo 栈、导出按 chunk 读 |

### 验收门禁

```sh
cd excel/rust/excel-core && cargo test --lib
cd excel/rust/wasm && cargo build
cd /Volumes/work/self/einfach && npx jest excel/solid-excel/test/sheet-store.test.ts
cd /Volumes/work/self/einfach && npx jest excel/solid-excel/test/worker-workbook-store.test.ts
cd excel/solid-excel && npx playwright test e2e/range-ops.spec.ts e2e/million-demo.spec.ts
```
...
`excel/rust/docs/WAVE3_IMPORT_PERSISTENCE_PLAN.md`。本波按多 agent pipeline 执行：
...
| 角色 | 推荐模型 | 文件所有权 | 任务 |
|---|---|---|---|
| C1 Import runtime | Codex Spark + 总架构师 | `excel/solid-excel/src/wasm-workbook-worker.ts`, tests | bounded session、错误码、cancel/commit 语义 |
| C2 Persistence schema | Codex Spark + 总架构师 | `excel/rust/wasm/src/lib.rs`, `wasm-workbook-proxy/worker` | sparse persistence v1 schema + worker API + round-trip |
| C3 Demo/import UI | 后续 | `excel/solid-excel/src/demos/DemoMillion.tsx` | 正式文件导入进度 UI 延后，不阻塞本波 API 合同 |
| C4 Tests/MCP | Codex Spark + 总架构师 | `excel/solid-excel/test/*`, `excel/solid-excel/e2e/worker-workbook.spec.ts` | lazy import、cancel、limit、persistence round-trip、MCP |

### 验收门禁

```sh
cd excel/rust/excel-core && cargo test --lib bulk_load
cd excel/rust/excel-core && cargo test --test cross_sheet
cd excel/rust/wasm && cargo test
cd excel/rust/wasm && cargo build
cd /Volumes/work/self/einfach && npx jest excel/solid-excel/test/wasm-workbook-worker.test.ts
cd excel/solid-excel && npx playwright test e2e/worker-workbook.spec.ts e2e/million-demo.spec.ts
```

本轮实际已跑：

```sh
cd excel/rust/wasm && cargo test --quiet                           # 21 passed
cd /Volumes/work/self/einfach && npx tsc -p excel/solid-excel/tsconfig.json --noEmit
cd /Volumes/work/self/einfach && npx jest excel/solid-excel/test/wasm-workbook-proxy.test.ts excel/solid-excel/test/wasm-workbook-worker.test.ts excel/solid-excel/test/worker-workbook-store.test.ts --runInBand  # 50 passed
cd /Volumes/work/self/einfach && npm run build:wasm -w @einfach/solid-excel
cd /Volumes/work/self/einfach && npm run build -w @einfach/solid-excel
cd excel/solid-excel && npm run e2e -- e2e/worker-workbook.spec.ts --grep "persistence|import"  # 9 passed
```
...
| 角色 | 推荐模型 | 文件所有权 | 任务 |
|---|---|---|---|
| D1 Rust counters | Codex Spark | `excel/rust/excel-core/src/*`, `excel/rust/wasm/src/lib.rs` | 补 counters 暴露和 unit tests |
| D2 Browser perf | Claude Sonnet | `excel/solid-excel/e2e/*`, MCP 验收脚本/说明 | viewport DOM 数、long task、heap、subscription bound |
| D3 Docs gate | Codex Spark | `excel/solid-excel/docs/E2E_TEST_PLAN.md`, `excel/rust/docs/HANDOFF.md` | 更新真实状态、测试矩阵、MCP 流程 |
...
```sh
cd excel/rust/excel-core && cargo test --lib
cd excel/rust/excel-core && cargo test --test scale
cd excel/rust/wasm && wasm-pack test --headless --chrome .
cd /Volumes/work/self/einfach && npx jest
cd excel/solid-excel && npm run build:wasm && npx playwright test e2e/million-demo.spec.ts e2e/virtualize.spec.ts e2e/worker-workbook.spec.ts
```
...
`excel/rust/docs/WAVE5_FILE_IMPORT_BACKPRESSURE_PLAN.md`。
...
| 角色 | 推荐模型 | 文件所有权 | 任务 |
|---|---|---|---|
| E1 流式 parser/helper | Codex Spark | `excel/solid-excel/src/file-import.ts`, `excel/solid-excel/test/file-import.test.ts` | stream parser、chunk flush、abort/cancel、progress 测试 |
| E2 导入 UI | 总架构师 + Codex Spark | `DemoMillion.tsx`, `styles.css`, i18n | 1M worker demo 文件导入控件、进度、取消、统计 |
| E3 E2E/MCP | Codex Spark | `excel/solid-excel/e2e/file-import.spec.ts` | 导入/取消/lazy eval/console 验收 |
...
```sh
cd /Volumes/work/self/einfach && npx tsc -p excel/solid-excel/tsconfig.json --noEmit
cd /Volumes/work/self/einfach && npx jest excel/solid-excel/test/file-import.test.ts excel/solid-excel/test/wasm-workbook-proxy.test.ts excel/solid-excel/test/wasm-workbook-worker.test.ts --runInBand
cd /Volumes/work/self/einfach && npm run build -w @einfach/solid-excel
...
- `excel/solid-excel/docs/E2E_TEST_PLAN.md` 当前已同步为 23 spec / 162 active Playwright tests /
  0 skip。
...
| 角色 | 推荐模型 | 文件所有权 | 任务 |
|---|---|---|---|
| E1 Formula diagnostics | Claude Sonnet | `excel/rust/excel-core/src/eval.rs`, parser 相关文件, `excel/rust/wasm/src/lib.rs` | 错误码/错误消息/diagnostics 合同 |
| E2 Multi-sheet product path | Codex Spark | `excel/solid-excel/src/wasm-workbook-store.ts`, sheet tabs 相关文件 | worker store 表级增删改和缓存一致性 |
| E3 UX/a11y | Codex Spark | `excel/solid-excel/src/Table.tsx`, `ContextMenu.tsx`, `FormatToolbar.tsx` | 虚拟列下键盘/菜单/工具条稳定 |
...
cd /Volumes/work/self/einfach && npm run build -w @einfach/solid-excel
cd /Volumes/work/self/einfach && npx jest
cd excel/solid-excel && npx playwright test
```
...
详细命令和 blocker 定义见 `excel/rust/docs/RELEASE_GATE_PLAN.md`。
...
cd excel/rust/excel-core && cargo test --test scale
cd excel/rust/excel-core && cargo test --test cross_sheet
cd excel/rust/excel-core && cargo bench --no-run
cd excel/rust/wasm && cargo test
cd excel/rust/wasm && cargo build
cd excel/rust/wasm && wasm-pack test --headless --chrome .
cd /Volumes/work/self/einfach && npx tsc -p excel/solid-excel/tsconfig.json --noEmit
cd /Volumes/work/self/einfach && npm run build -w @einfach/solid-excel
cd /Volumes/work/self/einfach && npx jest
cd excel/solid-excel && npm run build:wasm && npx playwright test
```

注意：当前 repo 没有统一 root Cargo workspace gate；完整 release gate 应优先按
`excel/rust/docs/RELEASE_GATE_PLAN.md` 里的 `--manifest-path` 命令执行。`excel-core` clippy 有历史
...
本波已在 `e5a25d0` 记录到 `excel/rust/docs/RELEASE_GATE_PLAN.md`：

- Rust core / excel-core / wasm native tests 通过。
- `cargo bench --manifest-path excel/rust/excel-core/Cargo.toml --no-run` 通过。
- `wasm-pack test --headless --chrome excel/rust/wasm` 通过。
