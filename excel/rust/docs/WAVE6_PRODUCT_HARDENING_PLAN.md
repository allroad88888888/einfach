...
| 轨道 | 推荐执行者 | 文件所有权 | 目标 |
|---|---|---|---|
| W6-A Worker 多 sheet store | Codex Spark / worker 子 agent | `excel/solid-excel/src/wasm-workbook-store.ts`, `excel/solid-excel/test/worker-workbook-store.test.ts` | 暴露 async `addSheet` / `renameSheet` / `removeSheet` / `indexOf`；结构变更后重建 sheet adapter/store，避免删除 sheet 后索引漂移 |
| W6-B SheetTabs + MultiSheet 产品路径 | Codex Spark / worker 子 agent | `excel/solid-excel/src/SheetTabs.tsx`, `excel/solid-excel/src/demos/MultiSheet.tsx`, i18n 必要文案 | `SheetTabs` 支持 sync/async workbook；`MultiSheet` 改为 worker-backed seed，移除“cross-sheet unsupported”旧文案 |
| W6-C E2E + MCP 用例 | Codex Spark / worker 子 agent | `excel/solid-excel/e2e/multisheet-ui.spec.ts` 或新增 worker spec, `excel/solid-excel/e2e/helpers.ts` 必要小改 | 钉 worker-backed multi-sheet：seed、tab、add/rename/delete、cross-sheet formula 和 lazy cache probe |
| W6-D 公式诊断与函数门禁规划 | Claude Sonnet / 只读审查优先 | `excel/rust/docs/*`, `excel/solid-excel/docs/E2E_TEST_PLAN.md` | 列出公式错误 UI、`TEXT/TODAY/NOW` 浏览器测试、虚拟列交互测试的下一批落点 |
...
cd /Volumes/work/self/einfach && npx jest excel/solid-excel/test/worker-workbook-store.test.ts excel/solid-excel/test/wasm-workbook-proxy.test.ts excel/solid-excel/test/wasm-workbook-worker.test.ts --runInBand
...
- Wave 6.5 虚拟化 UX 追补：
  ...
  - 本地门禁：`npx tsc -p excel/solid-excel/tsconfig.json --noEmit`、`npm run build -w
    @einfach/solid-excel`、`NO_PROXY=localhost,127.0.0.1 npm --prefix excel/solid-excel run e2e --
    e2e/context-menu.spec.ts`（5/5）、`NO_PROXY=localhost,127.0.0.1 npm --prefix excel/solid-excel
