...
| C-7 | FIXED (defensive) | Owner approved the core change 2026-06-13: `pendingMap.clear()` inside `clear()`. Fix-pass finding: not observable at the current public API (sub() pre-flushes; reads are cache-first) — landed as a defensive fence with a clear-drops-pending test. See § C-7 for the honesty note. | `core/core/src/store.ts`, `core/core/test/store.test.ts` |
| D-6 | **FIXED** | `removeSheet` / `moveSheet` shift the sheet indices captured by import/export/snapshot sessions and engine cell subscriptions — later chunks read/write the WRONG sheet, dirty events report stale indices. Both ops now route `invalidateSheetIndexedState` (sessions cleared → next RPC fails loudly with `*_SESSION_MISSING`; engine tokens unsubscribed). `addSheet`/`renameSheet` keep indices stable and deliberately do not invalidate (pinned). | `excel/solid-excel/src-vnext/adapter/worker-runtime.ts`, `excel/solid-excel/test/wasm-workbook-worker.test.ts` |
| D-9 | **FIXED** | Every single-cell write scanned ALL `readFormulaCells` entries (all sheets) with `startsWith` — O(host-read formulas) per keystroke. Re-keyed as `Map<sheetIdx, Set<'r:c'>>`: invalidation is one `Map.delete`, and the pin asserts a write does NOT over-invalidate other sheets' read state. | `excel/solid-excel/src-vnext/adapter/worker-runtime-ts.ts`, `excel/solid-excel/test/audit-adapter-scaling.test.ts` |
...
| D-11 | **FIXED** | `getConditionalFormatForCell` re-allocated + re-sorted the rule list per projected cell — O(window × rules·log rules) + a window's worth of array allocations per read. Sort hoisted into `applyConditionalFormatOverlay` (once per overlay application); behavior unchanged (stable sort, same priority order), covered by the existing conditional-format suites. | `excel/solid-excel/src-vnext/adapter/worker-workbook-backend.ts` |
| D-12 | SUPERSEDED | Fixed in W3: `replaceAllCappedAtom` + the "Replaced first N of M" dialog surface; unit-pinned in `excel/spreadsheet-ui-core/test/find-replace.test.ts`. | — |
...
`npx jest excel/solid-excel --no-coverage` 58 suites / 876 passed (incl. the
...
`excel/rust/excel-core/tests/audit_structural_scaling.rs` (5 always-on tests
...
## C — TS port (excel/excel-core-ts + core store)

Audit date 2026-06-12. Read-only at audit time; measurement pins live
in `excel/excel-core-ts/test/audit-mutation-scaling.test.ts`
...
Silicon, Node via jest, `npx jest excel/excel-core-ts --no-coverage`
...
- **FIXED** (W2.2, `d98409c`): storage-primary mutate-in-place — each
  ...
  (`excel/excel-core-ts/docs/KEY_GRANULAR_INVALIDATION.md`).
- `excel/excel-core-ts/src/sheet.ts:272` — `applyCell` does
...
- **FIXED** (W2.2, `d98409c`): the fix sketch's second option landed —
  sub-key dependency granularity, core/core untouched. Formula
...
- `core/core/src/store.ts:222-239` (`dependenciesChange`) +
...
- **CLOSED** (W3, docs-only — registry breadth is by design): the five
  ...
  `excel/excel-core-ts/docs/KEY_GRANULAR_INVALIDATION.md` § C-3
...
- `excel/excel-core-ts/src/workbook.ts:342-349`
...
- WAS: `excel/solid-excel/src-vnext/adapter/worker-runtime-ts.ts` `clearRange`
...
- WAS: `excel/excel-core-ts/src/workbook.ts` `withBatch` catch: on
...
- FIX (W1.4, commit `a62927d`): rollback — the outermost `withBatch`
  ...
  `excel/excel-core-ts/test/workbook.test.ts` (withBatch describe);
...
- **codex P1 follow-up** (post-`d98409c`, two findings, both fixed):
  ...
  `excel/excel-core-ts/test/key-granular-regressions.test.ts`
  (held-atom overwrite chain, mutual/3-cell/self/range-aggregate
  cycles).
- `excel/excel-core-ts/src/sheet.ts:137` `formulaAtomCache` and
  `:153` `lastEvalRevision` grow monotonically and have no eviction;
  `core/core` has no per-atom destroy/evict API (only whole-store
...
- Fix sketch: when a derive observes its cell no longer has an AST,
  ...
  and a new `store.evict(atom)` in core/core that runs
...
- `core/core/src/store.ts:277-282`: `clear()` replaces the four
...
- **FIXED** (W3): the fix sketch as written — `bulkApply` now accepts
  ...
  flipped: `excel/excel-core-ts/test/audit-mutation-scaling.test.ts`
  (typed-entry preservation) + the C-8 describe in
  `excel/solid-excel/test/audit-adapter-scaling.test.ts` (`'00123'`/`'TRUE'`/
  `'=A1'`/`'#N/A'` survive import and sheet-op round trips as text).
  Original finding below.

- `excel/solid-excel/src-vnext/adapter/worker-runtime-ts.ts` `importCells`
...
`excel/solid-excel/test/audit-adapter-scaling.test.ts` (loose assertions,
console timings — not perf gates; bug pins assert CURRENT buggy behavior
and say so inline). `npx jest excel/spreadsheet-ui-core excel/solid-excel
--no-coverage` green with the pins included (111 suites / 1636 tests).

Territory: `excel/spreadsheet-ui-core/src` (atoms, ports) and
`excel/solid-excel/src-vnext/adapter` (static backend, worker backends, TS/WASM
...
- **FIXED** (W2.4, `e507222`): the runtime now makes ONE call to the
  ...
  behavior tests in `excel/excel-core-ts/test/workbook.test.ts`.
- WAS:
- `excel/solid-excel/src-vnext/adapter/worker-runtime-ts.ts:692-704` — nested
...
- Reachable at catastrophic scale: a column-header selection produces
  ...
  (`excel/spreadsheet-ui-core/src/selection/index.ts:140-146`), and
...
- **FIXED** (W3): `beginUndoableMutation` no longer snapshots the
  ...
  (`excel/solid-excel/test/audit-adapter-scaling.test.ts`): 20 single-cell
  edits = **0.2 ms @50 cells vs ~0.0 ms @20k cells** (was 0.5 ms vs
  57.3 ms, 108×); behavior pinned by the new undo/redo delta suite in
  `excel/solid-excel/test/vnext-adapter.test.ts` + the audit-history /
  toolbar-history / undo-redo e2e specs.

- `excel/solid-excel/src-vnext/adapter/static-backend.ts:274-333` —
...
- **FIXED** (P3 triage 2026-06-13): `removeSheet` / `moveSheet` — the
  ...
  Pinned in `excel/solid-excel/test/wasm-workbook-worker.test.ts` (audit D-6
...
- **FIXED** (W3): the page-at-a-time cap stays (bounded find cache,
  ...
  close. Unit-pinned in `excel/spreadsheet-ui-core/test/find-replace.test.ts`.
...
`excel/rust/excel-core/tests/audit_restore_atoms_scaling.rs` (5 always-on
...
- **FIXED** (W2.3, `1d71ecf`): `restore_persistence_v1` now groups its
  ...
  (`sparse_cells_to_install_payload` in `excel/rust/wasm/src/lib.rs`) and
...
- `restore_sparse` / `restore_persistence_v1` route through
  ...
  (`excel/rust/wasm/src/lib.rs:3450` `restore_sparse_cells`), NOT the Phase 6
...
- (pre-fix analysis) Every primitive install allocated a core atom: `sheet.rs:1408`
  ...
  (`excel/rust/core/src/store.rs:43`, `:194-198`). Reads pay a double lookup
...
- `restore_sparse` (`excel/rust/wasm/src/lib.rs:3002`) applies records onto
...
- Worker RPC `readCells`
  (`excel/solid-excel/src-vnext/adapter/worker-runtime.ts:1559-1565`) maps
...
- **recalc / F9**: no recompute-everything API exists in `excel-core`
  or `excel/rust/wasm` (only "recalc" hit is the `INFO("recalc")` literal,
