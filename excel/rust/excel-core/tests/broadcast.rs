...
//! `excel/rust/excel-core/src/eval.rs`; this file drives it through the real
//! `Sheet` / `Workbook` API so we exercise the full spill plumbing
//! (`expr_may_produce_array` gate, anchor write, derived-atom install,
//! per-cell reads).
//!
//! See `excel/rust/excel-core/src/eval.rs` § `broadcast_binop` for the
//! per-cell semantics and `excel/rust/excel-core/src/sheet.rs` §
