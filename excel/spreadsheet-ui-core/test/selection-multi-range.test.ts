...
describe('multi-range selection', () => {
  ...
  test('arrow key moves primary region only; non-primary regions stay put', () => {
    ...
    expect(regionsBefore[1]).toEqual({
      kind: 'cell',
      sheetId: 'S1',
      anchor: { row: 7, col: 7 },
      focus: { row: 7, col: 7 },
  ...
  test('single-region path produces identical output to prior implementation', () => {
    ...
    expect(store.getter(primarySelectionRegionAtom)).toEqual({
      kind: 'range',
      sheetId: 'S1',
      anchor: { row: 2, col: 1 },
      focus: { row: 4, col: 3 },
  ...
  describe('setMultiRegionSelectionAtom', () => {
    ...
    test('rejects invalid primaryIndex values without changing the current selection', () => {
      const store = createStore()
      store.setter(setSelectionBoundsAtom, { rowCount: 20, colCount: 20 })
      ...
      const regionsBefore = store.getter(selectionRegionsAtom)
      const witnessBefore = store.getter(selectionAuthorityWitnessAtom)

      for (const invalidPrimaryIndex of [-1, 2, 0.5, Number.NaN, '1']) {
        store.setter(setMultiRegionSelectionAtom, {
          regions,
          primaryIndex: invalidPrimaryIndex as number,
        })
        expect(store.getter(selectionAtom)).toBe(selectionBefore)
        expect(store.getter(selectionRegionsAtom)).toBe(regionsBefore)
        expect(store.getter(selectionAuthorityWitnessAtom)).toBe(witnessBefore)
      }
    })
