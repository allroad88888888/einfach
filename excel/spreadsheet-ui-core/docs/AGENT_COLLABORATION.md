...
- `excel/spreadsheet-ui-core` 仍必须保持框架无关：不能依赖 Solid、React、DOM、worker、
...
| 2026-05-15 | Codex | multi-range UI integration | done | `excel/solid-excel/src-vnext/grid/SpreadsheetGrid.tsx`, `excel/solid-excel/test/vnext-grid.test.tsx` | Escape 已走 core intent；后续可接多 region command iteration |
...
| 2026-05-15 | Codex | rich-types UI integration | done | `excel/solid-excel/src-vnext/*`, `excel/solid-excel/test/vnext-*`, `excel/solid-excel/e2e/vnext-smoke.spec.ts` | 下一步可接 toolbar/context-menu 的 rich edit 入口 |
| 2026-07-14 | CC | Wave 8.2 custom formulas — async（sync MVP 此前已落地） | done | `excel/rust/core`+`excel/rust/excel-core`+`excel/rust/wasm`（`#BUSY!`、per-call memo atom、drain/resolve）、`excel/solid-excel/src-vnext/adapter/*`（共享 pump、两套 worker runtime）、`src/custom-formulas/*`（isAsync）、`src/backend/types.ts`（`registerCustomFormula` options + `subscribeContentChanges`）| 契约见 `excel/rust/excel-core/src/CUSTOM_FORMULAS.md` § Async；future work：TTL/手动刷新（RTD 语义）、`valueKind:'pending'` 样式、call-key 哈希化。Wave 8 余项 8.1 remote / 8.4 截图 / 8.5 copy-as-PNG 未开工，8.1 设计需对齐已落地的 drain/resolve 机制 |
...
- `excel/spreadsheet-ui-core/src/*` 的 feature core 实现。
- 对应 `excel/spreadsheet-ui-core/test/*` 的核心单测。
- feature doc 与实现之间的契约同步。

Codex 优先负责：

- 架构 review、状态建模 review、跨 feature 冲突检查。
- `excel/solid-excel/src-vnext/*` adapter / UI 接线。
...
- 是否会影响 `excel/solid-excel/src-vnext` adapter；如果会，先列出 adapter 后续任务。
...
```bash
git diff --check -- excel/spreadsheet-ui-core/docs
```
...
```bash
npx jest excel/spreadsheet-ui-core/test/selection-multi-range.test.ts \
  excel/spreadsheet-ui-core/test/selection.test.ts \
  excel/spreadsheet-ui-core/test/keyboard.test.ts \
  excel/spreadsheet-ui-core/test/package-boundary.test.ts --runInBand
```
...
- P1: Ctrl/Cmd multi-select is not reachable through the pointer contract yet.
  ...
  and has no modifier or append-mode field (`src/pointer/types.ts:27`). `excel/solid-excel`
  still calls `selectCellAtom` on cell click, so Ctrl/Cmd-click currently replaces the selection
  instead of calling `addSelectionRegionAtom` (`excel/solid-excel/src-vnext/grid/SpreadsheetGrid.tsx:1242`).
...
- `excel/solid-excel/test/vnext-grid.test.tsx`
...
- `excel/spreadsheet-ui-core/src/keyboard/*`
- `excel/spreadsheet-ui-core/src/pointer/*`
- `excel/spreadsheet-ui-core/test/selection-multi-range.test.ts`
- `excel/spreadsheet-ui-core/test/pointer.test.ts`
- `excel/spreadsheet-ui-core/docs/multi-range-selection.md`
...
npx jest excel/spreadsheet-ui-core/test/selection-multi-range.test.ts \
  ...
  excel/spreadsheet-ui-core/test/keyboard.test.ts \
  excel/spreadsheet-ui-core/test/pointer.test.ts \
  excel/spreadsheet-ui-core/test/package-boundary.test.ts \
  excel/solid-excel/test/vnext-grid.test.tsx --runInBand
NO_PROXY=localhost,127.0.0.1 npm run e2e -w @einfach/solid-excel -- e2e/vnext-smoke.spec.ts
```
...
- `excel/solid-excel/src-vnext/adapter/static-backend.ts`
- `excel/solid-excel/src-vnext/grid/SpreadsheetGrid.tsx`
- `excel/solid-excel/src-vnext/demos/VNextSmokeDemo.tsx`
- `excel/solid-excel/e2e/vnext-smoke.spec.ts`
- `excel/solid-excel/test/vnext-adapter.test.ts`
- `excel/solid-excel/test/vnext-grid.test.tsx`
...
npx tsc -p excel/spreadsheet-ui-core/tsconfig.json --noEmit --pretty false
npx tsc -p excel/solid-excel/tsconfig.json --noEmit --pretty false
npx jest excel/spreadsheet-ui-core/test/rich-types.test.ts \
  excel/spreadsheet-ui-core/test/package-boundary.test.ts \
  excel/solid-excel/test/vnext-adapter.test.ts \
  excel/solid-excel/test/vnext-grid.test.tsx --runInBand
NO_PROXY=localhost,127.0.0.1 npm run e2e -w @einfach/solid-excel -- e2e/vnext-smoke.spec.ts
