...
- **jsdom.** The Jest environment has no real `<canvas>` paint pipeline.
  ...
  `excel/solid-excel` Playwright e2e in a follow-up arc.
...
- `excel/spreadsheet-ui-core/test/copy-as-image.test.ts`
...
- `excel/solid-excel` host-level smoke test (follow-up) — assert the
...
- `excel/spreadsheet-ui-core/src/backend/types.ts` — add request /
  result types + optional method (~30 LOC).
- `excel/spreadsheet-ui-core/src/copy-as/types.ts` — widen
  `CopyAsResult` union (~10 LOC).
- `excel/spreadsheet-ui-core/src/copy-as/encodeSelectionAsImage.ts` —
  new (~30 LOC).
- `excel/spreadsheet-ui-core/src/copy-as/index.ts` — re-export (~2 LOC).
- `excel/spreadsheet-ui-core/test/copy-as-image.test.ts` — new
  (~80 LOC).
- `excel/solid-excel/src-vnext/copy-as/` — new dir with PoC adapter
...
7. ✅ Playwright e2e `excel/solid-excel/e2e/copy-as-png.spec.ts` asserts
