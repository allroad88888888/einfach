...
(`excel/rust/excel-core/src/sheet.rs` § Spill). 8.1 Remote formulas and
...
`Value` (in `excel/rust/core/src/atom.rs`) gains `Pending(PendingId)`. `PendingId`
...
`eval_func` (in `excel/rust/excel-core/src/eval.rs`) gets a `"REMOTE"` arm that
...
`FormulaCache` (in `excel/rust/excel-core/src/sheet.rs`) grows a
...
`excel/rust/wasm/src/lib.rs` adds:
...
`excel/solid-excel/src-vnext/adapter/worker-protocol.ts` adds three request kinds:
...
`excel/spreadsheet-ui-core/src/formula-remote/atoms.ts`:
...
> contract: `excel/rust/excel-core/src/CUSTOM_FORMULAS.md` (engine + § "Async
> custom formulas (Wave 8.2)") and
> `excel/spreadsheet-ui-core/src/custom-formulas/README.md` (host API).
...
>   worker pump (`excel/solid-excel/src-vnext/adapter/async-custom-pump.ts`,
...
E2E in `excel/solid-excel`: export `A1:D10` on a deterministic seed; assert blob
...
Rust engine (~1460 LOC): `excel/rust/core/src/atom.rs` (~80, new `Value`
variants + `ValueError::Remote`, `ValueError::Spill`),
`excel/rust/excel-core/src/eval.rs` (~600, `REMOTE`, dynamic-array funcs,
host-call fallback, array propagation), `excel/rust/excel-core/src/formula.rs`
(~80, `Expr::Implicit`), `excel/rust/excel-core/src/sheet.rs` (~250, spill
writer, pending cache), `excel/rust/excel-core/src/workbook.rs` (~200, pending +
host registries), `excel/rust/wasm/src/lib.rs` (~250, drain / fulfill / reject,
array wire).
...
- E2E (`excel/solid-excel/e2e/`): `formula-remote.spec.ts`, `formula-custom.spec.ts`,
