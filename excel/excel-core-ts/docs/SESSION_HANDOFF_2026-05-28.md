...
- `excel/solid-excel/e2e/helpers.ts::gotoRoot` preserves `?backend=` from project name
...
- Matrix in `excel/solid-excel/e2e/BACKEND_PARITY.md`
...
NOT backend issues. Touch `excel/solid-excel/src-vnext/` if you go after these.
...
- Added a TS worker UI e2e regression in
  `excel/solid-excel/e2e/vnext-worker-ts.spec.ts` covering the `RATE` zero-root path,
...
```bash
npx jest --runTestsByPath excel/excel-core-ts/test/financial.test.ts \
  --no-coverage --runInBand -t "RATE|invalid basis"
npx jest --runTestsByPath \
  excel/excel-core-ts/test/parser.test.ts \
  excel/excel-core-ts/test/reference-functions.test.ts \
  --no-coverage --runInBand -t "dynamic references|spill references|dynamic-range endpoints"
npx tsc -p excel/excel-core-ts/tsconfig.json --noEmit
npx jest excel/excel-core-ts --no-coverage --runInBand
unset http_proxy https_proxy all_proxy HTTP_PROXY HTTPS_PROXY ALL_PROXY \
  && NO_PROXY=localhost,127.0.0.1 \
  npx playwright test e2e/vnext-worker-ts.spec.ts --project=ts \
    -g "formula compatibility regressions" # from excel/solid-excel
git diff --check
```
...
```bash
npx jest --runTestsByPath \
  excel/excel-core-ts/test/financial.test.ts \
  excel/excel-core-ts/test/phase8-engineering.test.ts \
  excel/excel-core-ts/test/text.test.ts \
  excel/excel-core-ts/test/phase8-text.test.ts \
  excel/excel-core-ts/test/reference-functions.test.ts \
  excel/excel-core-ts/test/lambda-higher-order.test.ts \
...
- Post-Claude verification:
  - `npx jest --runTestsByPath excel/excel-core-ts/test/financial.test.ts excel/excel-core-ts/test/math.test.ts excel/excel-core-ts/test/text.test.ts --no-coverage --runInBand -t "RATE|ROUND|TEXT"`:
...
npx jest --runTestsByPath \
  excel/excel-core-ts/test/reference-functions.test.ts \
  excel/excel-core-ts/test/lambda-higher-order.test.ts \
  excel/excel-core-ts/test/lambda.test.ts \
  --no-coverage --runInBand
npx jest excel/excel-core-ts --no-coverage --runInBand
npx tsc -p excel/excel-core-ts/tsconfig.json --noEmit
unset http_proxy https_proxy all_proxy HTTP_PROXY HTTPS_PROXY ALL_PROXY \
...
```bash
npx jest --runTestsByPath excel/excel-core-ts/test/workbook.test.ts \
  --no-coverage --runInBand -t "array formula returns #SPILL|writing into a cached spill range"
npx jest --runTestsByPath \
  excel/excel-core-ts/test/workbook.test.ts \
  excel/excel-core-ts/test/reference-functions.test.ts \
  excel/excel-core-ts/test/scale-suite.test.ts \
  excel/excel-core-ts/test/audit-mutation-scaling.test.ts \
  --no-coverage --runInBand
npx tsc -p excel/excel-core-ts/tsconfig.json --noEmit
npx jest excel/excel-core-ts --no-coverage --runInBand
unset http_proxy https_proxy all_proxy HTTP_PROXY HTTPS_PROXY ALL_PROXY \
  && NO_PROXY=localhost,127.0.0.1 \
  npx playwright test e2e/vnext-worker-ts.spec.ts --project=ts \
    -g "formula compatibility regressions|SEQUENCE spill projects" # from excel/solid-excel
```
