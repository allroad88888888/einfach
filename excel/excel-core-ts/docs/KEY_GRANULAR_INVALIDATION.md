...
Findings: `excel/rust/excel-core/docs/AUDIT_PATTERN_FAMILY_2026-06-12.md` § C.
Measurement pins: `excel/excel-core-ts/test/audit-mutation-scaling.test.ts`.
...
2. **the change signal** — a fresh `new Map(prev)` per write was the only
   way to tell core/core "something changed", which made EVERY cached
...
dependency index; **core/core is untouched** (no new API, no behavior
change for `core/react-form`, `solid/*`, grid-table consumers).
...
- On a value-affecting write, the workbook runs a **dirty BFS** from
  ...
  has a cached atom. core/core's normal flush then re-derives
...
- Eager-at-mutation semantics are preserved (dependents re-derive
  ...
  `excel/solid-excel/test/excel-core-ts-debug-probes.test.ts` stay accurate.
