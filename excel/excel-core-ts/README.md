...
parallel atom-like dep graph that lives inside `excel/rust/excel-core/src/sheet.rs`, lets the reactive layer be unified across worker + UI, and turns `cargo build` + `wasm-pack` into a hot-reload TS edit.
...
The package must run cleanly under plain node (`jest --no-coverage` from repo root). Adapters that need a worker / DOM live downstream in `excel/solid-excel/src-vnext/adapter/`.
