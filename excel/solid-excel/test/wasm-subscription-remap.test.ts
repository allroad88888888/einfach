...
// the notification — see excel/rust/wasm/src/lib.rs). Flush with a macrotask
...
async function loadWasmModule(): Promise<WasmModuleShape> {
  ...
  if (!existsSync(WASM_PKG_JS) || !existsSync(WASM_PKG_BIN)) {
    throw new Error(
      `wasm-subscription-remap: wasm-pkg missing at ${WASM_PKG_JS} — run \`npm --prefix excel/solid-excel run build:wasm\``,
    )
  }
