 ...
 * `excel/rust/excel-core` (via the wasm-pack output in `excel/solid-excel/wasm-pkg/`)
...
describePerf('TS vs WASM perf bench (EINFACH_PERF=1)', () => {
  beforeAll(async () => {
    if (!existsSync(WASM_PKG_JS) || !existsSync(WASM_PKG_BIN)) {
      wasmSkipReason = `wasm-pkg missing at ${WASM_PKG_JS}; run \`npm --prefix excel/solid-excel run build:wasm\` first`
      return
    }
    try {
