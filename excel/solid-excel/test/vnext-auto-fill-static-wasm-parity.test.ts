 ...
 * now has two independent implementations — `excel/rust/excel-core/src/auto_fill.rs` (reached via
 * `excel/rust/wasm`'s `apply_auto_fill` and `worker-workbook-backend.ts`) and the parallel TS
 ...
 * engine — several are lifted verbatim from `excel/rust/excel-core/src/auto_fill.rs`'s own unit
...
describe('static ⇄ WASM auto-fill golden parity', () => {
  ...
  test('10. copy-fill propagates the source cell format to every target', async () => {
    ...
    // WASM wire's `NumberFormat` deserializer (excel/rust/wasm/src/lib.rs) only
