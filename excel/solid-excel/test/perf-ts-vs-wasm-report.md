...
> Generated by `excel/solid-excel/test/perf-ts-vs-wasm.bench.ts` when the
...
`resolveCell` → `evaluate` (see `excel/excel-core-ts/src/eval/evaluate.ts`).
...
`excel/rust/excel-core/src/sheet.rs` or `workbook.rs` triggers it directly.
...
[`excel/rust/wasm/src/lib.rs`](../../rust/wasm/src/lib.rs):
...
`excel/solid-excel && npm run build:wasm`):
...
| `cargo test --lib` in `excel/rust/excel-core` | 1356 passed | 1356 passed |
| `cargo test --lib` in `excel/rust/wasm` | 27 passed + 2 pre-existing stale-test compile errors | 29 passed (incl. 2 new guard tests; opportunistically repaired the stale `CellFormatJSON` fixtures with `..Default::default()`) |
...
`excel/rust/excel-core/src/sheet.rs`. Both panicked with `thread ... has
overflowed its stack` before the fix.

**Fix**: pre-warm pass at the top of
`eval_formula_at_with_provider` in
[`excel/rust/excel-core/src/sheet.rs`](../../rust/excel-core/src/sheet.rs).
...
- `cargo test --lib` in `excel/rust/excel-core`: **1360 passed / 0 failed**
  (was 1357 before adding three new chain regression tests).
- `cargo test --lib` in `excel/rust/wasm`: **29 passed / 0 failed**.
- New regression tests in `excel/rust/excel-core/src/sheet.rs`:
...
Pinned by `chain_bulk_install_is_linear` in `excel/rust/excel-core/src/sheet.rs`
