 ...
 * the Rust `sort_cmp` tests in `excel/rust/excel-core/src/sort.rs`. These lock the
...
describe('sort comparator — normative order (Rust sort_cmp parity)', () => {
  ...
  test('text compares by CODE POINT, not UTF-16 code unit', () => {
    ...
    // '\xEF\xBF\xBF' < '😀'. A naive UTF-16 comparison would order the astral char
    // FIRST (its lead surrogate 0xD83D < 0xFFFF) — this pins code-point order.
    expect(sign(compareSortText('\xEF\xBF\xBF', '\u{1F600}', true))).toBe(-1)
    expect(sign(compareSortText('\u{1F600}', '\xEF\xBF\xBF', true))).toBe(1)
  })
