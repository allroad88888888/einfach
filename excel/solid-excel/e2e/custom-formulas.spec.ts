...
test.describe('custom formulas — coverage notes', () => {
  ...
  // coverage lives in `excel/rust/excel-core/tests/` (re-entrancy + memory
  // limits) and `excel/spreadsheet-ui-core/test/custom-formulas.test.ts`
  ...
  test.fixme('re-entrancy guard rejects malicious sources', async () => {
    ...
    // Unit coverage: `excel/rust/excel-core/tests/custom_formulas.rs`.
  })
})
