...
| UI-core 代码边界                                              | `excel/spreadsheet-ui-core/**`；31 项有直接实现和测试、4 项部分实现；当前 `/root` 全量为 build PASS + 57/57 suites、1437/1437 tests；本轮前历史快照为 56/56、1432/1432（历史 C0 基线 55/1253、capacity 55/1261、projection 55/1274），不等于产品完成                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| Solid 代码边界                                                | `excel/solid-excel/src-vnext/**` 与直接测试；当前 `/root` full `--silent` 为 70 suites passed / 1 skipped（71 total）、1125 tests passed / 6 skipped（1131 total）、0 failed、exit 0；本轮前历史快照为 69+1 suites、1122+6 tests；既有 Vite build 证据为 PASS（293 modules；历史 projection 基线为 61+1 / 966+6）                                                                                                                                                                                                                                                                                                                                                                                 |
| Full Solid `tsc`                                              | `npx tsc --noEmit --pretty false -p excel/solid-excel/tsconfig.json` exit 2 / 恰好 5 diagnostics：`worker-runtime-ts.ts` 864、1306×2、1312，`worker-runtime.ts` 264；均为禁止扩围 baseline，不声明 PASS                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| engine 保护边界                                               | `core/core/**`、`excel/excel-core-ts/**`、`excel/rust/**` 不属于此次 UI parity 迁移                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| 文档边界                                                      | `excel/solid-excel/docs/online-excel-parity/**`；只记录代码证据，不反向驱动未授权源码变更                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
该限定路径现为 bounded `MAIN_REVIEW_ACCEPTED`：独立 reviewer **4 suites / 144 tests PASS**；`/root` 主审通过 adapter **99/99**、fill **17/17**、scaling **16/16**；该 bounded 包接受时的历史 Solid full 快照为 **69 suites passed / 1 skipped（70 total）**、**1080 tests passed / 6 skipped（1086 total）**；当前权威 Solid full 为 **70 suites passed / 1 skipped（71 total）**、**1125 tests passed / 6 skipped（1131 total），0 failed**，Vite build **PASS**。Full Solid `tsc` 仍恰好有 5 条禁止扩围的 worker baseline diagnostics，不能写 PASS。接受只覆盖 #12 `fillSeries` 的 plan/no-op/preflight、单 mutation/单 revision 与 undo/redo witness；不得外推为 Static 全局 history/no-op 原子性完成，generic Static same-value/no-op history 仍是独立债务。bounded per-cell fallback 已有引用平移，但完整 formula-series、Worker/真实 transport parity、date/weekday/month/custom、可见命令与系统门禁均未实现。#12 继续 `Partial`；严格 **41 = 0/35/5/1** 不变，数据分析/打印仍完全延后且在 41 项外。本次文档同步不 commit/push，也不触碰 `core/core`、`excel/excel-core-ts` 或 Rust。
...
| released | `/root/docs_evidence_refresh`                      | `excel/solid-excel/docs/online-excel-parity/**`                                   | `Docs Evidence MAIN_REVIEW_ACCEPTED / released`；只交文档 diff 与检查                                                                                                                                                                                                                       |
...
| released | #29 sort 显示置换彻底退役（任务 #24）              | directives 类型/支路/两 adapter 全清 + 排序入口 capability 门控             | ui-core 62 suites / 1484 tests、solid 92 suites / 1350 tests、e2e 18 passed / 4 skipped；ui-core `tsc` 0、solid `tsc` 5（既有 baseline，均在 worker-runtime\*）；#29 严格 Verified 判据满足，产品行翻转待主控                                                                               |
| released | #29 筛选隐藏语义重做（任务 #27，S1–S9）           | 压缩→隐藏原子翻转 + 引擎双隐藏集 + 可见性 UI-core canonical + 导出/删除收口 + `Data → Reapply` 入口 | 九提交（`7c13fd4`…`50a14e1`）+ S8 文档收口（`5a08eca`）+ S9 Reapply 补齐；S9 门禁：ui-core 64 suites / 1604 tests、solid 100 suites / 1472 tests（1 suite / 6 tests skipped）、e2e 1082 passed / 90 skipped / 0 failed；**#27 已无在案能力缺口**；#29 仍 `Partial`（状态列翻 `Verified` 属主控决策）                                                                                        |
...
#29 filter/sort capability truth 限定包已 `MAIN_REVIEW_ACCEPTED`：独立门禁为 **3 suites / 108 tests PASS**。Worker 没有 `setFilterSort`；UI-core capability 明确为 unsupported，入口禁用，只读取 bounded canonical window；不得新增 Map/cache/overlay、行置换或假 revision。Static 能力保持原状。

#29 sort 显示置换**彻底退役**（任务 #24，2026-07-20）：`SortDirective`、`FilterSortState.directives`、`SetFilterSortRequest.directives` 类型删除；`runFilterSortEntrypointAtom` / `dispatchSortAtom` / `clearColumnSortAtom` / `compareFilterSortRowsWith` / `compareFilterSortValues` 移除；`buildFilterSortDisplayRows` 只保留 filter 可见性，**行序恒为源序**；`static-backend` 与 `worker-workbook-backend` 的 `setFilterSort` 只承载 `rules`，predicate 扫描只含 filter 列。排序唯一机制是 `runPhysicalSortAtom` → `sortRange`：无端口即 fail-closed，命令返回 `'unsupported'` 诊断且不写任何视图状态，工具栏 Sort 按钮与 filter dropdown 排序区按 `sortRangeSupportedAtom` 隐藏、Data 菜单 sortAsc/sortDesc 按 `'sortRange'` capability key 隐藏——**TS worker 上排序能力完全消失**（e2e 实证三处入口 count 0、数据未动、`range.sort` 历史 0）。门禁：ui-core **62 suites / 1484 tests PASS**、solid **92 suites / 1350 tests PASS**（6 skipped）、e2e **18 passed / 4 skipped**、ui-core `tsc` **0**、solid `tsc` **5**（既有 baseline，全部落在 `worker-runtime*.ts`，与本改动无关）、Prettier clean。至此 #29 的严格 Verified 三项判据（全入口物理排序 / 显示置换零残留 / 入口 capability 门控）**全部满足**；产品行是否由 `Partial` 翻 `Verified` 需同步改本文件与 CANONICAL_OWNERSHIP 的 0/35/5/1 总账，留主控决策。

#29 筛选**隐藏语义重做**（任务 #27，2026-07-21）：筛选从"匹配行压缩进连续 display 槽位 + `DisplayCell.originalRow` 回映射"翻为 Excel 的**隐藏行**——被筛行留在原位、行号跳号（1、4、7）、**display 序 ≡ 源序**，两个坐标系合并为一个。`originalRow` 字段与 W2 网关的整个 display→source 回映射半边（per-cell 源行回声、run 裂分范围映射、`unmapped-row` 全族、`requireIdentityMapping` 及其两调用点）随压缩一并删除；**protection 门禁半边全留**。UI-core 持两个**不可合并**的集合：`viewportHiddenAtom`（手动，有自己的 history）与 `viewportFilterHiddenAtom`（筛选，无独立 history，其 undo 即规则的 undo），两者落在 `excel/spreadsheet-ui-core/src/viewport/effective-hidden.ts`。并集 `effectiveHiddenAtom` **只服务渲染与导航**；凡是**要动数据**的消费者读筛选子集——复制 / Copy As / `exportRangeTsv` 分块 / `exportRangeAsImage`、`planFilterVisibleRowDeletions` 删除行、`remove-duplicates` 与 `text-to-columns` 稠密扫描、`buildSortExcludedRows`、SUBTOTAL 引擎推送。可见性由 adapter 在 `setFilterSort` 里**一次全列谓词扫描**算定（沿用既有 `MAX_FILTER_SORT_PREDICATE_CELLS = 50_000` 与 `FILTER_SORT_SOURCE_TOO_LARGE` fail-closed，零新阈值），随 ACK 经 `SetFilterSortResult.hiddenRowIndices` 回传 UI-core；**投影期不再做任何筛选计算**，`readFilteredRange` / `MappedDisplayRow` / display-row permutation 缓存整体删除，投影读退回矩形范围读。引擎侧新增**纯加法**端口 `setEvalFilterHiddenRows`（wasm 快照同 commit 重生成，INV-4 删改判据不触发）+ 第二个 per-sheet 隐藏集 + **双失效 epoch**，使 `SUBTOTAL(1-11)` 排除筛选行但**包含**手动隐藏行、`(101-111)` 两者都排除，且推送手动隐藏不脏化全工作簿 1-11 公式。

用户可见变化四条，两条需进发布说明：**粘贴/填充会写入被筛掉的行**（Excel 就是连续块覆写，是向 Excel 收敛的 parity **修复**，但对习惯旧行为的用户可感知）；**筛选不再实时**——改单元格值不会让行当场消失/出现（快照语义，与 Excel 一致，压缩语义反而比 Excel 更"活"）。另两条：行号跳号（产品目的）、筛选区合并单元格重新可见。**`Data → Reapply` 入口已于 S9 补齐（2026-07-21）**：`reapplyFilterAtom` + `reapplyFilterDisabledReasonAtom` + `data.reapply` 菜单项 + `Ctrl+Alt+L`。重算复用 adapter 的整列扫描（重发已提交规则），不进 undo 栈，只重做筛选不重做排序，无激活筛选时禁用而非隐藏 —— 三项裁决的理由与证据强度见 design-filter-hidden-rows §0.0 勘误 E8 与 §4.3。至此 #27 无在案能力缺口。

门禁：ui-core **63 suites / 1583 tests PASS**、solid **97 suites / 1455 tests PASS**（1 suite / 6 tests skipped）。两次运行在含无关 Table 在途改动的工作树上取得，非纯 #27 树。三条降级档全部无声且不撒谎：两端口齐全→完整两档语义；只有 `setEvalHiddenRows`（旧 wasm-pkg）→筛选集不进引擎，两档退化为改造前行为，**视图正确、公式偏保守，不产生错误数字**；两个都无（TS worker）→与既有 `evalHiddenRows: false` 同形。导出侧的隐藏集过滤刻意**不**走引擎推送而在主线程 adapter 边界生效，隐藏集从不跨 `postMessage`，因此 WASM / TS / static 三条路一致，无 capability 门控、无 wasm-pkg 版本偏斜风险。#29 产品行仍为 `Partial`。
...
| 路径                                                                                  | diff 哈希      | Owner                            | 处置                            |
| ------------------------------------------------------------------------------------- | -------------- | -------------------------------- | ------------------------------- |
| `excel/solid-excel/README.md`                                                               | `426d2164e8d7` | CC-B                             | owner 导入                      |
| `excel/solid-excel/src-vnext/conditional-formatting/SpreadsheetConditionalFormatDialog.tsx` | `5c51d9c65f02` | CC-B                             | owner 导入（W0-DIALOG Rework）  |
| `excel/solid-excel/src-vnext/data-validation/SpreadsheetDataValidationDialog.tsx`           | `139ad2f0cc81` | CC-B                             | owner 导入（W0-DIALOG Rework）  |
| `excel/solid-excel/src-vnext/find-replace/SpreadsheetFindReplaceDialog.tsx`                 | `b784e22b3423` | CC-B                             | owner 导入（W0-DIALOG Rework）  |
| `excel/solid-excel/src-vnext/name-box/SpreadsheetNameBox.tsx`                               | `cd56858ed9a3` | CC-B（含 CC-A 救援修复，见注 1） | owner 导入                      |
| `excel/solid-excel/src-vnext/protection/SpreadsheetProtectionUnlockDialog.tsx`              | `5deb4108d5a4` | CC-B                             | owner 导入（W0-DIALOG Rework）  |
| `excel/solid-excel/src/LocaleSwitcher.tsx`                                                  | `ee0e64573ca3` | CC-B                             | owner 导入（i18n 保全，不排期） |
| `excel/solid-excel/src/i18n/index.ts`                                                       | `d504a0de405f` | CC-B                             | owner 导入（i18n 保全，不排期） |
| `excel/spreadsheet-ui-core/src/conditional-formatting/index.ts`                     | `160855a75580` | CC-B                             | owner 导入（W0-DIALOG）         |
| `excel/spreadsheet-ui-core/src/conditional-formatting/types.ts`                     | `8f6bdc47a274` | CC-B                             | owner 导入（W0-DIALOG）         |
| `excel/spreadsheet-ui-core/src/data-validation/index.ts`                            | `0fff4ab5dbf2` | CC-B                             | owner 导入（W0-DIALOG）         |
| `excel/spreadsheet-ui-core/src/data-validation/types.ts`                            | `f5ee04f1d5cb` | CC-B                             | owner 导入（W0-DIALOG）         |
| `excel/spreadsheet-ui-core/src/find-replace/index.ts`                               | `9722dd9a662b` | CC-B                             | owner 导入（W0-DIALOG）         |
| `excel/spreadsheet-ui-core/src/find-replace/types.ts`                               | `c420a428ae0b` | CC-B                             | owner 导入（W0-DIALOG）         |
| `excel/spreadsheet-ui-core/src/name-box/index.ts`                                   | `3655bce84aa3` | CC-B                             | owner 导入（W0-DIALOG）         |
| `excel/spreadsheet-ui-core/src/protection/index.ts`                                 | `383080378428` | CC-B                             | owner 导入（W0-DIALOG）         |
| `excel/spreadsheet-ui-core/test/conditional-formatting.test.ts`                     | `72fcbc0dd2b5` | CC-B                             | owner 导入（W0-DIALOG）         |
| `excel/spreadsheet-ui-core/test/data-validation.test.ts`                            | `4d349b4af09f` | CC-B                             | owner 导入（W0-DIALOG，注 2）   |
| `excel/spreadsheet-ui-core/test/find-replace.test.ts`                               | `e712ffb7d20d` | CC-B                             | owner 导入（W0-DIALOG）         |
| `excel/spreadsheet-ui-core/test/protection.test.ts`                                 | `de8001ce08bb` | CC-B                             | owner 导入（W0-DIALOG）         |
...
- CC-A 隔离 worktree `excel-parity/cc-a-a05-f1`（基线 `2feea48`）现有 **1 个未提交预研文件**：`excel/excel-core-ts/src/rewrite/index.ts`（F-1 引用改写核心草稿）。它在 `/root` 裁决前动工，不计交付，但按当前裁决已真实占用执行槽并保持 `active / quarantined`；H0 与 cutover 关闭前不得进入 H1、提交交接或集成。
...
| DOCS / `/root` 文档主审与限定集成   |     16 | `excel/solid-excel/README.md` 加 parity 目录 15 个文件（含本文件）；与 W0 patch 分包，不能记作 CC-B 功能实现          |
