...
- **状态**：**E0–E8 已全量落地（2026-07-22），E9 文档收口完成。** 本文既是裁决记录、
  ...
  `excel/spreadsheet-ui-core/docs/filter-sort.md`（引擎拥有筛选、谓词在 Rust 求值）
...
- **范围**：把**隐藏行集合 + 筛选规则 + 筛选谓词求值**从宿主（UI-core / adapter）
  下沉到 `excel/rust/excel-core`。**隐藏列不动**（§8）。
...
> 正文均已标注 as-built。读现行契约优先看 `excel/spreadsheet-ui-core/docs/filter-sort.md`
...
>    `hidden` / `filters` 字段（`excel/rust/wasm/src/lib.rs`），闭合 xlsx parity 缺口。
...
>    UI-core 谓词零知识（`grep filterRuleMatchesValue excel/spreadsheet-ui-core/src` 零命中）。
...
> backend canonical，实际只活在 `static-backend` 的内存态；两个引擎（`excel/rust/excel-core`、
> `excel/excel-core-ts`）和 worker RPC 从未建模它们；**引擎没有任何公式读取 hidden
...
1. **引擎有了两个隐藏集，且是真实求值输入。**
   `excel/rust/excel-core/src/sheet.rs:947` `eval_hidden_rows`、`:954` `eval_filter_hidden_rows`。
2. **`SUBTOTAL` 真的读它们，且两档规则不同。**
   `excel/rust/excel-core/src/eval.rs:19694-19699` 定义 `SubtotalHiddenPolicy`
...
1. **结构位移免费跟随。** `Sheet::insert_row` / `delete_row` 已是 sheet 方法，
   ...
   而 wasm 的 `insert_row`（`excel/rust/wasm/src/lib.rs:2550-2551`）**已经**路由到
...
3. **持久化免费跟随。** persistence v1 的 sheet 元数据已按 sheet 组织
   （`excel/rust/wasm/src/lib.rs:1130` `sheets: Vec<WorkbookPersistenceSheetMetaJSON>`）。
...
- **预算下沉。** `MAX_FILTER_SORT_PREDICATE_CELLS = 50_000` 目前在 worker adapter
  ...
  （`excel/rust/wasm/src/lib.rs:1158-1167` 逐字描述了这个 convention）。
...
- 两条路真正的分歧在**取值器**：worker 读 `snapshot.display`，来自 Rust
  `value_to_display`（`excel/rust/wasm/src/lib.rs:3578`）；static 读
...
1. 谓词从 `excel/spreadsheet-ui-core/src/backend/projection-helpers.ts`
   **迁出**到 `excel/solid-excel/src-vnext/adapter/`（比如 `static-filter-predicate.ts`）。
...
3. **黄金对照钉死两者。** 照抄 `excel/solid-excel/test/vnext-sort-static-wasm-parity.test.ts`
...
- **隐藏状态完全不进引擎快照。** `WorkbookPersistenceV1JSON`
  （`excel/rust/wasm/src/lib.rs:1128-1146`）只有 `version` / `sheets` / `cells` /
...
（`excel/rust/wasm/src/lib.rs:1136-1145` 的注释把双向后向兼容的理由写全了：
...
- **筛选侧**：`VIEWPORT_FILTER_HIDDEN_REPLAY_KEY` + applier
  ...
  可全删，引擎快照覆盖。
- **手动侧不能删。** 已核实：`excel/spreadsheet-ui-core/src/outline/index.ts:13-14`
...
| 文件                                                       | 区间                        | 内容                                                        | 约行数 |
| ---------------------------------------------------------- | --------------------------- | ----------------------------------------------------------- | -----: |
| `excel/solid-excel/src-vnext/provider/eval-hidden-rows-bridge.ts` | 全文                        | 手动隐藏推送桥（含 `WeakMap` 单实例 owner、`lastPushed` ledger） |    120 |
...
| `excel/solid-excel/src-vnext/adapter/filter-hidden-rows.ts`      | 1-56                        | `filterHiddenRowsFromDisplayRows`（排列→隐藏集折叠）        |     56 |
| `excel/solid-excel/src-vnext/adapter/worker-workbook-backend.ts` | 2192-2198, 2209-2262        | `filterSortPredicateColumns` + `computeFilterSortDisplayRows`（含 50k 预算） |     71 |
...
| `excel/solid-excel/src-vnext/adapter/static-backend.ts`          | 1276-1320                   | `filterHiddenRowsForSheet` + `shiftFilterHiddenRows`        |     45 |
...
**核实**：`excel/rust/excel-core/src/` 与 `excel/rust/wasm/src/` 全仓对 "hidden" 的命中，
...
**T6 已经落地**：`excel/rust/wasm/src/lib.rs:2550-2551` 的 `WasmWorkbook::insert_row`
...
| 片      | 范围                                                                                                            | 文件边界                                                                                     | 验收门禁                                                                                                     |
| ------- | --------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ |
| **E0**  | **前置修复 D1**：`sync_atom_topology` 挂 `republish`，让隐藏集随 sheet 增删移重键                                | `excel/rust/excel-core/src/workbook.rs` 单文件                                                     | 新增 Rust 单测：sheet 0 删除后 sheet 1 的 101-111 仍读自己的集合。**零 TS 改动、零 wasm 签名变化**             |
| **E1**  | **outline 解耦**：给大纲折叠自己的 replay key，停止复用 `VIEWPORT_HIDDEN_REPLAY_KEY`                            | `vanilla/…/src/outline/index.ts` + `README.md`                                               | `npx jest excel/spreadsheet-ui-core --no-coverage` 全绿；折叠 undo 单测显式断言用的是新 key                  |
| **E2**  | **Rust 拥有隐藏行（手动侧）**：`Sheet.hidden_rows` + hide/unhide/list + 结构跟随 + `republish_hidden`（含幂等去重）+ `snapshot_hidden`/`restore_hidden` + persistence 字段。`set_eval_hidden_rows` 保签名，改为写拥有态 | `excel/rust/excel-core/src/{sheet,workbook}.rs`、`excel/rust/wasm/src/lib.rs`、`wasm_api_signatures.txt` | `cargo test -p einfach-excel-core -p einfach-wasm`；INV-4 同提交重生成且 diff 只见新增；**空集合 sheet 插入行两个 epoch 均不 bump** 的单测 |
...
| **E4**  | **谓词迁出 UI-core**：`projection-helpers.ts` 的谓词族搬到 `src-vnext/adapter/static-filter-predicate.ts`，static 独用 | `vanilla/…/src/backend/projection-helpers.ts`、`excel/solid-excel/src-vnext/adapter/`              | 两包单测全绿；`grep -r filterRuleMatchesValue excel/spreadsheet-ui-core/src` **零命中**                       |
| **E5**  | **协议与 adapter 转发**：新增 capability 键 + 两 runtime dispatch + `readSheetHiddenState` 端口；worker adapter 的 `computeFilterSortDisplayRows` / `pushEvalFilterHiddenRows` / `filterHiddenRowsBySheetId` 三层全删 | `worker-protocol.ts`、`worker-runtime.ts`、`worker-runtime-ts.ts`、`worker-workbook-backend.ts` | `npx jest excel/solid-excel --no-coverage`；TS runtime 显式 `false` 且入口隐藏（fail-closed，不得假 ACK）             |
...
| `excel/solid-excel/test/vnext-structural-remap-static.test.ts` 的 filter-hidden 部分 | adapter 本地快照与其平移一起消失                          |
...
| `excel/solid-excel/test/vnext-worker-filter-subtotal-wasm.test.ts`     | **最重要的保留项**：两档 SUBTOTAL 的真实 Rust 验证，本设计不得让它变弱 |
...
- `excel/solid-excel/test/vnext-filter-static-wasm-parity.test.ts`（E6，同提交）。
- undo 往返三类（E8）。
