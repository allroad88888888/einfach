...
Gate: `excel/solid-excel/e2e/a11y-surfaces.spec.ts`
...
| Driver | `@axe-core/playwright` 4.12.1, added to `excel/solid-excel` devDependencies |
...
`excel/solid-excel/src-vnext/formula-bar/SpreadsheetFormulaBar.tsx`
...
`excel/solid-excel/src-vnext/history/SpreadsheetHistoryTimeline.tsx`
...
`excel/solid-excel/src-vnext/format-cells/format-cells-dialog.css`
...
npx tsc -p excel/solid-excel/tsconfig.json --noEmit      → 5 errors (unchanged
  pre-existing baseline, all in src-vnext/adapter/worker-runtime*.ts)
npx jest excel/solid-excel --no-coverage                 → 94 suites / 1398 passed
