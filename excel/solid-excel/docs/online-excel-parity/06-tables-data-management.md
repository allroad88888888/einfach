...
9. **筛选语义为 Excel 的隐藏行，不是显示压缩（#27，2026-07-21 落地）**：被筛行留在原位、行号跳号、display 序 ≡ 源序。由此产生两条**用户可感知**的行为变化，本组任何后续切片都必须按此为准：**粘贴与填充照写被筛掉的行**（Excel 的连续块覆写，是 parity 修复而非回归），**筛选不再实时**——改单元格值不会让行当场消失或出现（快照语义）。配套的 `Data → Reapply`（`Ctrl+Alt+L`）**尚未实现**，今天只能靠重开列下拉重发规则刷新。完整契约见 [design-filter-hidden-rows.md](./design-filter-hidden-rows.md) 与 `excel/spreadsheet-ui-core/docs/filter-sort.md`。
