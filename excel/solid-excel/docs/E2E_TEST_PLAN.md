...
**Fix** (`excel/rust/wasm/src/lib.rs`): defer the JS callback to a microtask
on `wasm32` targets:
...
`excel/rust/wasm/tests/web.rs` (5 `#[wasm_bindgen_test]`, see TODO 2.3 ✅).
`wasm-pack test --headless --chrome excel/rust/wasm` exercises the
`queueMicrotask` defer path AND the panic-inject knob (C.10) end-to-end
in a real browser; `wasm-pack test --node excel/rust/wasm` is the fallback
...
`excel/solid-excel/README.md` if not already there. CI is unaffected.
...
- `excel/solid-excel/playwright.config.ts`
...
3. Shared helper functions should live in `excel/solid-excel/e2e/helpers.ts`.
...
Implemented in `excel/solid-excel/e2e/helpers.ts`. Exports:
...
Change `excel/solid-excel/playwright.config.ts` web server command to ensure WASM is
    ...
    steps:
      ...
      - uses: actions/upload-artifact@v4
        ...
        with:
          ...
          path: excel/solid-excel/playwright-report/
          retention-days: 14
```
...
Create `excel/solid-excel/e2e/workbook-chain.spec.ts`.
...
Create `excel/solid-excel/e2e/selection-clipboard.spec.ts`.
...
Create `excel/solid-excel/e2e/formulas-wasm.spec.ts`.
...
Create `excel/solid-excel/e2e/formula-bar.spec.ts`.
...
Implemented in `excel/solid-excel/e2e/multisheet-ui.spec.ts`. Original plan
...
Create `excel/solid-excel/e2e/render-counter.spec.ts`.
...
```bash
npx playwright test -c excel/solid-excel/playwright.config.ts workbook-chain.spec.ts
```
