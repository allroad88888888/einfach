...
`excel/solid-excel/src` 上补丁式演进，而是把在线电子表格的核心能力收敛到
`excel/solid-excel/src-vnext`、`@einfach/spreadsheet-ui-core`、Rust/wasm workbook 这三层。
...
- W1 已完成：真实 worker runtime 已迁到
  `excel/solid-excel/src-vnext/adapter/worker-runtime.ts`，vNext worker factory 指向 vNext
  runtime；legacy `excel/solid-excel/src/wasm-workbook-worker.ts` 只保留兼容 shim。
...
| --- | --- | --- | --- | --- |
| W1 | 已完成：vNext worker 实现边界迁移 | Worker Runtime、Worker Tests、Public Surface Review | `excel/solid-excel/src-vnext/adapter/*`、legacy worker shim、worker tests/e2e | vNext 不再指向 legacy worker 实现；Jest worker tests；vNext worker e2e；MCP smoke |
| W2 | 已完成：Lazy formula 正确性回归 | Rust Lazy、Wasm Contract、E2E Lazy | `excel/rust/excel-core/*`、`excel/rust/wasm/*`、WASM/JS tests、lazy e2e | 单 sheet dependent 传播；dirty notify 契约；TLS resolver 清零；vNext lazy probe + MCP |
...
- 子 agent：Rust/Wasm Scale（Codex `gpt-5.5` xhigh，写 `excel/rust/wasm/src/lib.rs`）；
  Worker Import Tests（Codex `gpt-5.5` xhigh，写 `excel/solid-excel/test/vnext-adapter.test.ts`）；
...
- 已跑测试：`npx tsc -p excel/solid-excel/tsconfig.json --noEmit --pretty false`；
  `npx jest excel/solid-excel/test/wasm-workbook-proxy.test.ts excel/solid-excel/test/wasm-workbook-worker.test.ts excel/solid-excel/test/worker-workbook-store.test.ts excel/solid-excel/test/vnext-adapter.test.ts --runInBand`；
  `cargo test`（`excel/rust/wasm`）；`wasm-pack test --headless --chrome .`（`excel/rust/wasm`）；
  `NO_PROXY=localhost,127.0.0.1 npm run e2e -w @einfach/solid-excel -- e2e/vnext-worker-backend.spec.ts`；
  `npm run build -w @einfach/solid-excel`。
...
- 后续追加记录：已跑 `npx tsc -p excel/spreadsheet-ui-core/tsconfig.json --noEmit --pretty false`、
  `npx tsc -p excel/solid-excel/tsconfig.json --noEmit --pretty false`、
  `npx jest excel/spreadsheet-ui-core/test/clipboard.test.ts excel/solid-excel/test/vnext-context-menu.test.tsx excel/solid-excel/test/vnext-adapter.test.ts --runInBand`、
  `NO_PROXY=localhost,127.0.0.1 npm run e2e -w @einfach/solid-excel -- e2e/vnext-worker-backend.spec.ts`、
  `npm run build -w @einfach/solid-excel`。
- direct import 追加记录：已跑
  `npx tsc -p excel/solid-excel/tsconfig.json --noEmit --pretty false`；
  `npx jest excel/solid-excel/test/wasm-workbook-worker.test.ts excel/solid-excel/test/wasm-workbook-proxy.test.ts excel/solid-excel/test/vnext-adapter.test.ts excel/solid-excel/test/worker-workbook-store.test.ts --runInBand`；
  `NO_PROXY=localhost,127.0.0.1 npm run e2e -w @einfach/solid-excel -- e2e/vnext-worker-backend.spec.ts`；
  `npm run build -w @einfach/solid-excel`。
